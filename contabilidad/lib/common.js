...
// FUNCION PARA TERCEROS
function cargar_grupo()
{
	var grupo=document.getElementById('grupo1')
	var auxiliar=document.getElementById('auxiliar')
	var contenido_tipotipo=abrirAjax()
	  contenido_tipotipo.open("GET", "../paginas/grupos.php?opcion="+grupo.value, true)
   	  contenido_tipotipo.onreadystatechange=function()
	{
		if (contenido_tipotipo.readyState==4)
		{
		    alert(contenido_tipotipo.responseText);
			auxiliar.innerHTML = contenido_tipotipo.responseText;
		}
	}
	contenido_tipotipo.send(null);
}