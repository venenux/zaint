...
include ("header.php");
	...
	left join cwcondcohis as dco on cue.Cuenta=dco.Cuenta  where cue.Tipo='p' 
...
{ 
    ...
    {  	
		...
		{
		$consulta="SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha >='$Fecini' AND Fecha <'$fecha1'";
       ...
       $Salantu   = $row_ant["Debsum"] - $row_ant["Credsum"];
		...
		$consult="SELECT * FROM cwcondcohis WHERE Cuenta='$Cuenta_bucle' AND Fecha BETWEEN '$fecha1' AND '$fecha2' ORDER BY FechaD,Referen";
		...
		{
		$result_subtotal = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha BETWEEN '$Fecini' AND '$fecha1'", $conectar); 
         $row_subtotal = @mysql_fetch_array($result_subtotal); 
         $Salantu_tot   = $row_subtotal["Debsum"] - $row_subtotal["Credsum"];
		}
         $result_subtotal = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha BETWEEN '$fecha1' AND '$fecha2'", $conectar); 
         $row_subtotal = @mysql_fetch_array($result_subtotal); 
	 ...
	 $Credito_float_format = ((string)$Credito_float_format);
         ...
         $result_subtotal = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha BETWEEN '$Fecini' AND '$fecha2'", $conectar); 
         $row_subtotal = @mysql_fetch_array($result_subtotal); 
