...
if(isset($_POST['buscar']) || $tipob!=NULL){
	...
	switch($tipob){
		case "exacta": 
			...
			break;
		case "comienzo":
				$consulta=buscar_comienzo($tabla,$des,$busqueda);
				break;
		case "todas":
		...
		//echo "cod: ".$id;
		$consulta="select * from cwconcue order by Cuenta";
		//echo $consulta;
}
...
<?
	titulo($modulo,"cwconcueedit.php?accion=agregar","","12");
?>
</FORM>
<table class="tb-head" width="100%">
  <tr>
	...
	<td width="150"><input onclick="javascript:actualizar(this);" type="radio" name="palabra" value="comienzo">Comenzando Por</td>
...
<?
	...
	icono("cwconhislist.php?Cuenta=".$cuenta."&pagina=".$pagina, "Historico de Cuentas","70.png");
	...
	$buscar='select * from cwconcue where cuenta like "'.$cuenta.'%" and Nivel='.($nivel+1);
	$resul_buscar=query($buscar,$conexion);
	$buscar2='select * from cwcondco where Cuenta="'.$cuenta.'"';
	$resul_buscar2=query($buscar2,$conexion);
	

	if(num_rows($resul_buscar)==0){
		if(num_rows($resul_buscar2)==0){
			icono("javascript:confirmar('Desea Borrar la Cuenta  Contable? .$cuenta','cwconcueedit_sql.php?Cuenta=".$cuenta."&pagina=".$pagina."&Accion=Borrar')" , "Borrar Cuentas","ico_basket.gif");
		}else{ echo "<td></td>";}
	}else{
		echo "<td></td>";
	}
