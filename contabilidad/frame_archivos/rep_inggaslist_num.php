...
Sistema de contabilidad.        <strong>ESTADO DE INGRESOS Y GASTOS DESDE <strong><?php echo $fecha1?></strong> AL <?php echo $fecha2?></strong>                                      
...
<?php 
   ...
   {  	
     ...
     $result_ant = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND Fecha>='$Fecini' AND Fecha<'$fecha11'", $conectar); 
     ...
     $result_sum = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND Fecha BETWEEN '$fecha11' AND '$fecha22'", $conectar); 
   ...
   $result   = mysql_query("SELECT * FROM cwconaux WHERE Cuenta='$Baling_pas' ORDER BY Cuenta ASC", $conectar); 
   $row      = @mysql_fetch_array($result);
   $Baling_2 = $row["Baling"]; //INGRESOS

   $result   = mysql_query("SELECT * FROM cwconaux WHERE Cuenta='$Baleng_pas' ORDER BY Cuenta ASC", $conectar); 
   $row      = @mysql_fetch_array($result);
   $Baleng_2 = $row["Baleng"];  //GASTOS

   $Resultado  = $Baleng_2 - $Baling_2;
