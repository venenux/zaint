	...
	//echo $result;
   	...
   	{  	
		...
		else
		{
			...
			if($mes33==12)
			{
				$iniciomes=$anio."-".$mes33."-01";	
				$numdiasmes = date("t",strtotime($iniciomes));
				$fechahastasalant=$anio."-".$mes33."-".$numdiasmes;	
				//echo "SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta%' AND Fecha BETWEEN '$iniciomes' AND '$fechahastasalant'"; 
				$result_ant = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta%' AND Fecha BETWEEN '$iniciomes' AND '$fechahastasalant'", $conectar); 
				//echo "<br>";
			...
			else
			{
				...
				$fechahastasalant=$anio."-".$mes33."-".$numdiasmes;	
				//echo "SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta%' AND Fecha BETWEEN '$iniciomes' AND '$fechahastasalant'"; 
				$result_ant = mysql_query("SELECT Salactu  FROM cwconhis WHERE Cuenta = '$Cuenta' AND Mes = month('$iniciomes') AND Anio=year('$fechahastasalant')", $conectar); 
				//echo "<br>";
				$row_ant = @mysql_fetch_array($result_ant); 
// 				$Credito = $row_ant["Credsum"];
// 				$Debito  = $row_ant["Debsum"];
				$Salantu = $row_ant['Salactu']; //$Debito - $Credito; //SALDO ANTERIOR HASTA LA FECHA
			}
		}
...
function imprimir_datos($pagina,$num_paginas,$fecha1,$fecha2,$Desde_cod,$Hasta_cod)
{
	...
	$imagen=$row_emp["imagen"];
	
	$datos_orden='<br>
	<table width="800" border="0" align="center">
		<tr>
			<td rowspan="3" align="center" valign="top"><img src="Imagenes/'.$imagen.'" width="100" height="60"></td>
...
$result_niv1 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='1' and (Debito<>0 or Credito<>0 or Salant<>0) ORDER BY Cuenta ASC", $conectar); 
...
{ 
  ...
  $result_niv2 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='2' and (Debito<>0 or Credito<>0 or Salant<>0)  AND Cuenta LIKE '$Cuenta_niv1%' ORDER BY Cuenta ASC", $conectar); 
  ...
  {  	
    ...
    $result_niv3 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='3' and (Debito<>0 or Credito<>0 or Salant<>0) AND Cuenta LIKE '$Cuenta_niv2%' ORDER BY Cuenta ASC", $conectar); 
    ...
    {  	
      ...
      $result_niv4 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='4' and (Debito<>0 or Credito<>0 or Salant<>0)  AND Cuenta LIKE '$Cuenta_niv3%' ORDER BY Cuenta ASC", $conectar); 
	...
	while ($row_niv4 = @mysql_fetch_array($result_niv4)) //NIVEL 4
      {  	
       ...
       $result_niv5 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='5' and (Debito<>0 or Credito<>0 or Salant<>0) AND Cuenta LIKE '$Cuenta_niv4%' ORDER BY Cuenta ASC", $conectar); 
        ...
        {  	
		 ...
		 $result_niv6 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='6' and (Debito<>0 or Credito<>0 or Salant<>0) AND Cuenta LIKE '$Cuenta_niv5%' ORDER BY Cuenta ASC", $conectar); 
		 ...
		 if($cont==$cantidad_registros)
		 {
		  ...
		  $result_niv7 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='7' and (Debito<>0 or Credito<>0 or Salant<>0)  AND Cuenta LIKE '$Cuenta_niv6%' ORDER BY Cuenta ASC", $conectar); 
		  ...
		  $result_niv8 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='8' and (Debito<>0 or Credito<>0 or Salant<>0) AND Cuenta LIKE '$Cuenta_niv7%' ORDER BY Cuenta ASC", $conectar);
		  ...
		  $result_niv9 = mysql_query("SELECT * FROM cwconaux WHERE Nivel='9' and (Debito<>0 or Credito<>0 or Salant<>0) AND Cuenta LIKE '$Cuenta_niv8%' ORDER BY Cuenta ASC", $conectar); 
