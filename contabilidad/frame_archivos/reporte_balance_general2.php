...
include ("header.php");
	...
	$Hoja = (empty($_REQUEST['hoja'])) ? '' : $_REQUEST['hoja'];
	...
	$Balact_pas    = $Hoja.$Sepacta; 
   	...
   	{  	
		...
		$result_sum = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND Fecha BETWEEN '$Fecini' AND '$fecha2'", $conectar); 
	...
	if($Hoja==2)
	{
		...
		{  	
			...
			{
			$result_ant = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND FechaD BETWEEN '$Fecini' AND '$fecha1'", $conectar); 
			$row_ant = @mysql_fetch_array($result_ant); 
			$Credito = $row_ant["Credsum"];
			$Debito  = $row_ant["Debsum"];
			$Salantu = $Debito - $Credito; //SALDO ANTERIOR HASTA LA FECHA
			}
			$result_sum = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND FechaD BETWEEN '$Fecini' AND '$fecha2'", $conectar); 
			$row_sum = @mysql_fetch_array($result_sum);
			$Credito = $row_sum["Credsum"];
			$Debito  = $row_sum["Debsum"];
			$Salmes  =  $Debito - $Credito;
	...
	//echo $num_paginas;
function imprimir_datos($pagina,$num_paginas,$fecha1,$fecha2,$Desde_cod,$Hasta_cod,$Nivcue)
{
	...
	<table width="800" border="0" align="center">
		<tr>
			<td rowspan="4" align="center" valign="top"><img src="Imagenes/lamas.png" width="100" height="60"></td>
		...
		<tr>
			...
			<td width="300" align="center" class="texto10"><strong>BALANCE GENERAL</strong></td>
		...
		<tr>
			...
			<td width="300" align="center" class="texto10">Al: '.fecha($fecha2).'</td>
...
imprimir_datos($pagina++,$num_paginas,$fecha1,$fecha2,$Desde_cod,$Hasta_cod,$Nivcue);
...
{ 
	...
	if($cont>=$cantidad_registros)
	{
		...
		$Fecini=imprimir_datos($pagina++,$num_paginas,$fecha1,$fecha2,$Desde_cod,$Hasta_cod,$Nivcue);
	...
	{
		datos_cuenta_p($Cuenta_niv1,$Descrip_niv1,$Salantu_float_format_niv1,$Debito_float_format_niv1,$Credito_float_format_niv1,$Salmes_float_format_niv1,$Salactu_float_format);
		$nro_lineas=contar_lineas($Descrip_niv1,35);
	...
	if ($Nivcue>=2) if ($Nivcue==2)
   	{
		datos_cuenta_p($Cuenta_niv2,$Descrip_niv2,$Salantu_float_format_niv2,$Debito_float_format_niv2,$Credito_float_format_niv2,$Salmes_float_format_niv2,$Salactu_float_format);
		$nro_lineas=contar_lineas($Descrip_niv2,35);
	    $cont=$cont+$nro_lineas-1;
    }else
	{
		datos_cuenta_p($Cuenta_niv2,$Descrip_niv2,$Salantu_float_format_niv2,$Debito_float_format_niv2,$Credito_float_format_niv2,$Salmes_float_format_niv2,$Salactu_float_format);
		...
		{
		datos_cuenta_p($Cuenta_niv3,$Descrip_niv3,$Salantu_float_format_niv3,$Debito_float_format_niv3,$Credito_float_format_niv3,$Salmes_float_format_niv3,$Salactu_float_format); 
		$nro_lineas=contar_lineas($Descrip_niv3,35);
	    $cont=$cont+$nro_lineas-1;
		} else
		{
		datos_cuenta_p($Cuenta_niv3,$Descrip_niv3,$Salantu_float_format_niv3,$Debito_float_format_niv3,$Credito_float_format_niv3,$Salmes_float_format_niv3,$Salactu_float_format); 
	    ...
	    $cont=$cont+$nro_lineas-1;
			...
			{
			datos_cuenta_p($Cuenta_niv4,$Descrip_niv4,$Salantu_float_format_niv4,$Debito_float_format_niv4,$Credito_float_format_niv4,$Salmes_float_format_niv4,$Salactu_float_format);
			$nro_lineas=contar_lineas($Descrip_niv4,35);
			$cont=$cont+$nro_lineas-1;
			} else
			{
			datos_cuenta_p($Cuenta_niv4,$Descrip_niv4,$Salantu_float_format_niv4,$Debito_float_format_niv4,$Credito_float_format_niv4,$Salmes_float_format_niv4,$Salactu_float_format); 
			$nro_lineas=contar_lineas($Descrip_niv4,35);
	  		$cont=$cont+$nro_lineas-1;
				...
				{
				datos_cuenta_p($Cuenta_niv5,$Descrip_niv5,$Salantu_float_format_niv5,$Debito_float_format_niv5,$Credito_float_format_niv5,$Salmes_float_format_niv5,$Salactu_float_format); 
				$nro_lineas=contar_lineas($Descrip_niv5,35);
	 			$cont=$cont+$nro_lineas-1;
				}else
		   		{
				datos_cuenta_p($Cuenta_niv5,$Descrip_niv5,$Salantu_float_format_niv5,$Debito_float_format_niv5,$Credito_float_format_niv5,$Salmes_float_format_niv5,$Salactu_float_format); 
				...
				{
					datos_cuenta_p($Cuenta_niv6,$Descrip_niv6,$Salantu_float_format_niv6,$Debito_float_format_niv6,$Credito_float_format_niv6,$Salmes_float_format_niv6,$Salactu_float_format); 
					$nro_lineas=contar_lineas($Descrip_niv6,35);
	 				$cont=$cont+$nro_lineas-1;				
				}else
				{
					datos_cuenta_p($Cuenta_niv6,$Descrip_niv6,$Salantu_float_format_niv6,$Debito_float_format_niv6,$Credito_float_format_niv6,$Salmes_float_format_niv6,$Salactu_float_format_niv); 
					$nro_lineas=contar_lineas($Descrip_niv6,35);
	 				$cont=$cont+$nro_lineas-1;
				}
				...
				{    	
					...
					{
					datos_cuenta_p($Cuenta_niv7,$Descrip_niv7,$Salantu_float_format_niv7,$Debito_float_format_niv7,$Credito_float_format_niv7,$Salmes_float_format_niv7,$Salactu_float_format); 
					$nro_lineas=contar_lineas($Descrip_niv7,35);
	 				$cont=$cont+$nro_lineas-1;
					}else
					{
					datos_cuenta_p($Cuenta_niv7,$Descrip_niv7,$Salantu_float_format_niv7,$Debito_float_format_niv7,$Credito_float_format_niv7,$Salmes_float_format_niv7,$Salactu_float_format); 
					...
					if ($Nivcue>=8) if ($Nivcue==8)
					{
						...
						$nro_lineas=contar_lineas($Descrip_niv8,35);
	 					$cont=$cont+$nro_lineas-1;
					}else 
					{
						datos_cuenta_p($Cuenta_niv8,$Descrip_niv8,$Salantu_float_format_niv8,$Debito_float_format_niv8,$Credito_float_format_niv8,$Salmes_float_format_niv8,$Salactu_float_format);
					...
					$Descrip_niv9 = $row_niv9["Descrip"];
						...
						$nro_lineas=contar_lineas($Descrip_niv9,35);
	 					$cont=$cont+$nro_lineas-1;
						}else 
						{
						datos_cuenta_p($Cuenta_niv9,$Descrip_niv9,$Salantu_float_format_niv9,$Debito_float_format_niv9,$Credito_float_format_niv9,$Salmes_float_format_niv9,$Salactu_float_format);
