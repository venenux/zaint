...
<?php 
 ...
 {
	//echo $Numcom_get;
    $result_max = mysql_query("SELECT MAX(Numlim) FROM cwcondco WHERE Numcom ='$Numcom_get'", $conectar); 
 ...
 } else if ($accion=='agregar_sub')
 {
    $Numlim = $_GET['Numlim'];
   	$Findebucle = 1;
    do {
         $result_do = mysql_query("SELECT COUNT(*) FROM cwcondco WHERE Numlim ='$Numlim'", $conectar); 
...
//echo "$var[2]-$var[1]-$var[0]";  
 ...
 if ($accion=='modificar')
 {
   ...
   $url_dinamico = 'cwcondcoedit_sql.php?RecNo='.$Asiento.'&pagina='.$pagina.'&Accion=Modificar';
 } else if ($accion=='agregar')
 {
   $valor_boton = 'Agregar';
   $url_dinamico = "cwcondcoedit_sql.php?Accion=Agregar&RecNo=".$Asiento.'&pagina='.$pagina;
 }
 ...
 if ($accion=='modificar')
 {
   ...
   $result = mysql_query("SELECT * FROM cwcondco WHERE RecNo ='$Asiento'", $conectar);
...
if ($accion=='agregar')
 {
 $result_sum = mysql_query("SELECT Sum(Debito) as suma_Debito FROM cwcondco WHERE Numcom='$Numcom_get '", $conectar); //SUMA DE DEBITOS DE LISTA DE ASIENTOS
 $Debito_total_array  = mysql_fetch_array($result_sum);

 $result_sum = mysql_query("SELECT Sum(Credito) as suma_Credito FROM cwcondco WHERE Numcom='$Numcom_get '", $conectar); //SUMA DE DEBITOS DE LISTA DE ASIENTOS
 $Credito_total_array = mysql_fetch_array($result_sum);
...
//echo "<br>".$Debito_total." ".$Credito_total;
   ...
   $Numcom  = $_GET['Numcom'];
   $result_max = mysql_query("SELECT MAX(RecNo) FROM cwcondco WHERE Numcom ='$Numcom_get'", $conectar); 
   $Asi = mysql_result($result_max,0,0);

   $result = mysql_query("SELECT * FROM cwcondco WHERE RecNo ='$Asi'", $conectar);
   $row = mysql_fetch_row($result);
...
?>
<?
titulo_mejorada("Asientos",'',"cwcondcolist.php?Numcom=$Numcom_get&pagina=$pagina","Imagenes/asientosjpg");
?>
<TABLE width="100%">
  <TBODY>
...
<option <?php if ($Tiporef == 'RV') echo 'selected' ?> value="RV">RV</option>
...
<tr><TD colspan="2" align="center">            
            ...
            </form><form action="cwcondcolist.php?RecNo=<?php echo $RecNo?>" method="get" name="Form_aux"><input name="Numcom" type="hidden" value="<?php echo $Numcom_get?>"><input name="RecNo" type="hidden" value="<?php echo $RecNo?>"><input name="pagina" type="hidden" value="<?php echo $pagina?>"></form>
            </TD>
          </TR>
