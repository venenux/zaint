...
include ("header.php");
	...
	$result_config = mysql_query("SELECT * FROM cwconfig", $conectar);
   	...
   	$Baling        = $array_config["Baling"]; //INGRESOS
   	$Baleng        = $array_config["Baleng"]; //GASTOS
    
   	$Sepacta       = $array_config["Sepacta"];
   	$Sepacta_len   = strlen($Sepacta);
   	$Baling_pas    = $Baling.$Sepacta; //INGRESOS
   	$Baleng_pas    = $Baleng.$Sepacta; //GASTOS

	$del_aux="DELETE FROM cwconaux WHERE Cuenta<>'Z'";
	$result = query($del_aux, $conectar);  
	//echo $result;
   	$result = mysql_query("SELECT * FROM cwconcue WHERE Cuenta LIKE '$Baleng_pas%' ORDER BY Cuenta DESC", $conectar); 
	...
	$result = mysql_query("SELECT * FROM cwconcue WHERE Cuenta LIKE '$Baling_pas%' ORDER BY Cuenta DESC", $conectar);
     ...
     //QUERY PARA EL SALDO ANTERIOR
     $result_ant = mysql_query("SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta%' AND Fecha>='$Fecini' AND Fecha<'$fecha1'", $conectar); 
     $row_ant = @mysql_fetch_array($result_ant); 
     $Credito = $row_ant["Credsum"];
     $Debito  = $row_ant["Debsum"];
     $Salantu = $Debito - $Credito; //SALDO ANTERIOR HASTA LA FECHA
...
function imprimir_datos($pagina,$num_paginas,$fecha1,$fecha2,$Desde_cod,$Hasta_cod,$Nivcue)
{
	...
	<table width="800" border="0" align="center">
		...
		<tr>
			...
			<td width="300" align="center" class="texto10"><strong>ESTADO DE INGRESOS Y GASTOS</strong></td>
