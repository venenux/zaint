...
<?
titulo("Tipos de auxiliares","","cwconterlist_asiento.php?Numcom=$Numcom&Asiento=$row[RecNo]&pagina=$pagina&feccom=$feccom","12");
?>
...
function paginar($actual, $total, $por_pagina, $enlace){
  ...
  $result = mysql_query("SELECT COUNT(*) FROM cwconter", $conectar); 
  ...
  if (($unico=='SI')&&($psearch<>'')) 
  {
    $result = mysql_query("SELECT Codtipo, Descrip, Rif, Nit FROM cwconter WHERE Codtipo='$psearch'", $conectar); 
  } else {
  */  
  ...
  // BUSQUEDA REALIZADA POR DESCRIPCION Y NO POR CODIGO.
    ...
    if ($order == 'Codtipo_ASC'){
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE ORDER BY Codtipo ASC LIMIT $reg1, $tampag", $conectar); 
    } else if ($order == 'Codtipo_DESC'){
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE ORDER BY Codtipo DESC LIMIT $reg1, $tampag", $conectar); 
    } else if ($order == 'Descrip_ASC'){
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE ORDER BY Descrip ASC LIMIT $reg1, $tampag", $conectar); 
    } else if ($order == 'Descrip_DESC'){
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE ORDER BY Descrip DESC LIMIT $reg1, $tampag", $conectar); 
	...
	// ** PARA EL CAMPO GRUPO
    {
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE WHERE grupo=codgrup ORDER BY Descrip DESC LIMIT $reg1, $tampag", $conectar); 
    } else if ($order == 'Grupo_ASC') 
    {
      $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE WHERE grupo=codgrup ORDER BY Descrip ASC LIMIT $reg1, $tampag", $conectar); 
    ...
    {
      $result = mysql_query("SELECT CC.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE WHERE grupo=codgrup LIMIT $reg1, $tampag", $conectar); 
    } 	 
  } else {
    $result = mysql_query("SELECT C.Codtipo, C.Descrip, C.Rif, C.Nit, RE.descrip FROM cwconter as C, cwcongrup as RE WHERE grupo=codgrup LIMIT $reg1, $tampag", $conectar); 
  }
    ...
    echo '<TABLE class=ewTable id=ewlistmain width="100%">'; // tabla externa
      ...
      while ($row = @mysql_fetch_array($result)){  
        echo "<tr class=ewTableAltRow onmouseover=ew_mouseover(this); onclick=ew_click(this); onmouseout=ew_mouseout(this);><td>".$row["Codtipo"]."</td><td>".$row["Descrip"]."</td><td>".$row["Rif"]."</td><td>".$row["Nit"]."</td><td>".$row["descrip"]."</td>"; 
          echo '<TD noWrap width="1%"><A href="cwconterlist_asiento.php?grupo='.$row["grupo"].'&Codtipo='.$row["Codtipo"].'&accion=guardar&Numcom='.$Numcom.'&Asiento='.$Asiento.'&pagina='.$pagina.'&feccom='.$feccom.'">';
		    echo '<IMG height=15 alt=Agregar src="usuarioslist_archivos/add.gif" width=15 border=0></A> </TD>';
...
if (($order == '')&&($unico<>'SI')&&($psearch=='')){
  echo paginar($pag, $num_tot, $tampag, "cwconterlist.php?pag=");
} else
{
  if ($order == 'Codtipo_ASC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Codtipo_ASC&pag=');
  } else  if ($order == 'Codtipo_DESC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Codtipo_DESC&pag=');
  } else  if ($order == 'Descrip_ASC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Descrip_ASC&pag=');
  } else  if ($order == 'Descrip_DESC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Descrip_DESC&pag=');
  } else  if ($order == 'Rif_ASC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Rif_ASC&pag=');
  } else  if ($order == 'Rif_DESC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Rif_DESC&pag=');
  } else  if ($order == 'Nit_ASC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Nit_ASC&pag=');
  } else  if ($order == 'Nit_DESC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Nit_DESC&pag=');
  } else  if ($order == 'Grupo_ASC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Grupo_ASC&pag=');
  } else  if ($order == 'Grupo_DESC'){
    echo paginar($pag, $num_tot, $tampag, 'cwconterlist.php?order=Grupo_DESC&pag=');
  }  
}
?> 
