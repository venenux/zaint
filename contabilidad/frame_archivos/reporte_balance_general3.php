...
include ("header.php");
	...
	//$mes_fin = (empty($_REQUEST['mes_fin'])) ? '' : $_REQUEST['mes_fin'];
...
function imprimir_datos($pagina,$num_paginas,$Nivcue,$mes,$ano)
{
	...
	<table width="800" border="0" align="center">
		<tr>
			<td rowspan="4" align="center" valign="top"><img src="Imagenes/ventel.png" width="100" height="60"></td>
		...
		<tr>
			...
			<td width="300" align="center" class="texto10">Mes: '.mesaletras($mes).'</td>
                        <td width="200" align="center" class="texto10">A&#241;o: '.$ano.'</td>
...
function datos_cuenta_jose($codigo,$descripcion,$saldoanterior,$debito,$credito,$saldomes,$saldoactual)
{	if($saldoactual<0)
	{$saldoactual=-1*$saldoactual;
	$saldoactual = ((real) $saldoactual);
  	$saldoactual  = number_format($saldoactual,2,',','.');
  ...
  {  	
	...
	/*		
          ...
          if ($Nivcue>=5) if($Nivcue==5)
	  {
	    if($Tipo_niv5=='P')
            {
	    datos_cuenta_jose($Cuenta_niv5,$Descrip_niv5,$Salantu_float_format_niv5,$Debito_float_format_niv5,$Credito_float_format_niv5,$Salmes_float_format_niv5,$Salactu_niv5); 
		...
		*/
	    ...
	    if ($Nivcue>=6) if($Nivcue==6)
	    {
	      //echo $Tipo_niv6;
              if($Tipo_niv6=='P')
              {
	      datos_cuenta_jose($Cuenta_niv6,$Descrip_niv6,$Salantu_float_format_niv6,$Debito_float_format_niv6,$Credito_float_format_niv6,$Salmes_float_format_niv6,$Salactu_niv6); 
 ...
 //echo $Salactu_niv7;
	    if ($Nivcue>=7) if($Nivcue==7)
	    {
	      //echo $Tipo_niv7;
              if($Tipo_niv7=='P')
              {
	      datos_cuenta_jose($Cuenta_niv7,$Descrip_niv7,$Salantu_float_format_niv7,$Debito_float_format_niv7,$Credito_float_format_niv7,$Salmes_float_format_niv7,$Salactu_niv7); 
	      $nro_lineas=contar_lineas($Descrip_niv7,60);
	      $cont=$cont+$nro_lineas;
	      }else if($Tipo_niv7=='T')
	      {
	      datos_cuenta_t($Cuenta_niv7,$Descrip_niv7,$Salantu_float_format_niv7,$Debito_float_format_niv7,$Credito_float_format_niv7,$Salmes_float_format_niv7,$Salactu_niv7); 
	      $nro_lineas=contar_lineas($Descrip_niv7,60);
	      $cont=$cont+$nro_lineas;
	      }	
	    }else
...
$result_niv1 = mysql_query("SELECT * FROM cwconaux WHERE Cuenta='2.'", $conectar);
while ($row_niv1 = @mysql_fetch_array($result_niv1))
{
	$Pasivo = $row_niv1["Salactu"];
}
$result_niv1 = mysql_query("SELECT * FROM cwconaux WHERE Cuenta='3.'", $conectar); //3.2.5.02.01.
while ($row_niv1 = @mysql_fetch_array($result_niv1))
{
	$Resultado = $row_niv1["Salactu"]; 
}
$pasres=$Pasivo+$Resultado;
	
$pasres_float = ((real) $pasres);
$pasres_float_format  = number_format($pasres_float,2,',','.');
$pasres_float_format_print  = ((string)$pasres_float_format);

datos_cuenta_pt($Cuenta_niv1,'PASIVO Y RESULTADOS',$Salantu_float_format_niv1,$Debito_float_format_niv1,$Credito_float_format_niv1,$Salmes_float_format_niv1,$pasres);
