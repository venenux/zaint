...
function datos_cuenta($codigo,$descripcion,$saldoanterior,$saldoactual)
{
	$datos_cuenta='<table width="800" border="0" align="center">
		<tr>
			...
			<td width="100">
			<td width="100">
			<td width="100" align="right" class="texto10"><strong></strong></td>
   		</tr>
	</table>';
echo $datos_cuenta;
}
...
{ 
    ...
    {  	
		...
		{
		//$consulta="SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwconhis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha >='$Fecini' AND Fecha <'$fecha1'";
		$consulta="SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondcohis WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha <='".$fecha1."'";
		$consulta1="SELECT SUM(Credito) AS Credsum, SUM(Debito) AS Debsum  FROM cwcondco WHERE Cuenta LIKE '$Cuenta_bucle%' AND Fecha <='".$fecha1."'";
		//QUERY PARA EL SALDO ANTERIOR
		$result_ant = mysql_query($consulta, $conectar); 
		$row_ant = @mysql_fetch_array($result_ant); 

		$result_ant1 = mysql_query($consulta1, $conectar); 
		$row_ant1 = @mysql_fetch_array($result_ant1); 
		$Salantu   = (($row_ant["Debsum"]+$row_ant1["Debsum"]) - ($row_ant["Credsum"]+$row_ant1["Credsum"]));
		$Salantuxxx=$Salantu;
		}
	 ...
	 $saldoactual = $Salantuxxx+($Debito-$Credito);

	//TOTALES GENERALES
        $salAntTotal+=$Salantuxxx;
	$debitoTotal+=$Debito;
	$creditoTotal+=$Credito;
	$salActTotal+=$saldoactual;
	 ...
	 $Credito_float_format = ((string)$Credito_float_format);
		 ...
		 $Salactu_float_format  = number_format($Salactu_float,2,',','.');
		// $Salantu_float_format = number_format($Salantu_float,2,',','.');
		$Salantu_float_format = number_format($Salantuxxx,2,',','.');
		$Salactu_float_format  = ((string)$Salactu_float_format);
		$Salantu_float_format = ((string)$Salantu_float_format);
	...
	echo '<table width="800" border="0" align="center">
				...
				<tr>
					...
					<td width="100" aling="right">'.number_format($salAntTotal,2,',','.').'</td>
					<td width="100" align="right" class="texto8">'.number_format($debitoTotal,2,',','.').'</td>
					<td width="100" align="right" class="texto8">'.number_format($creditoTotal,2,',','.').'</td>
					<td width="100" aling="right" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.number_format($salActTotal,2,',','.').'</td>
				</tr>
			  </table>'; 
	echo "<br>";
