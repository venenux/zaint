...
<?php 
 ...
 if ($accion=='modificar')
 {
   ...
   $url_dinamico = 'cwconccoedit_sql.php?Accion=Modificar';  
 } else if ($accion=='agregar')
 {
   $valor_boton = 'Agregar';
   $url_dinamico = 'cwconccoedit_sql.php?Accion=Agregar';
 } 
 ...
 if ($accion=='modificar')
 {
   ...
   $result = mysql_query("SELECT * FROM cwconcco WHERE Codccos ='$codigo'", $conectar); 
  ...
  <TR>
    <TD class=row-br>
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        <TBODY>
        <TR>
          <TD><SPAN style="FLOAT: left"><IMG class=icon height=35 src="Imagenes/Centros_de_Costos.jpg" 
            width=35>Centros de costo disponibles</SPAN></TD>
          <TD align=right>
...
C&oacute;digo de centro de costo: <?php
			   if ($accion=='modificar')
               {
                 echo "<input name=Codccos id='Codccos' value='$codigo' readonly size=10 onBlur='javascript:this.value=this.value.toUpperCase()'>";
               } else if($accion=='agregar')
               {
                 echo '<input name=Codccos id="Codccos" value="# NUEVO" readonly size=10 onBlur="javascript:this.value=this.value.toUpperCase()">';
               }        			
