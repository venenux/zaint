...

{
	...
	{
	try {if (variables[auxi].value !='' ) cadi = cadi  + variables[auxi].id + "=" + variables[auxi].value + "&"; } catch (error) {}
	}
	

	window.setTimeout(function () {miAjax.send(cadi);}, 100);
...
var idItemVideo = dest[1];
	...
	{var aux = document.getElementById(destino); 
		...
		try
 		 {eval(miAjax.responseText); }  catch(err) { aux.innerHTML=miAjax.responseText; }
...
if (document.cont>=2) {document.cont=document.cont-2;}
}
...
try {
		...
		{ aux2.hijoActivo.style.visibility='hidden'; aux2=aux2.hijoActivo;}
...
try {
		...
		{ aux.hijoActivo.style.visibility='hidden'; aux=aux.hijoActivo;}
