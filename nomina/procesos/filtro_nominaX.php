...
function direccionar_txt()
{
    ...
    if(opcion.value==1)
        ...
        parent.cont.location.href="txt_federal.php?codigo_nomina="+document.form1.cboTipoNomina.value
        ...
        parent.cont.location.href="orden_nomina_liq.php?codigo_nomina="+document.form1.cboTipoNomina.value
	else if(opcion.value==13)
        parent.cont.location.href="contabilizar_nomina_liq.php?codigo_nomina="+document.form1.cboTipoNomina.value+"&cod_banco="+document.form1.banco.value
}
</script>
...
elseif(($opcion==1)||($opcion==4)||($opcion==7)||($opcion==8)||($opcion==9)||($opcion==11))
{
?>
...
?>
    ...
    <option>Seleccione una <?echo $termino?></option>
    <?php
    echo $query="select codnom,descrip,codtip from nom_nominas_pago where codtip='".$_SESSION['codigo_nomina']."' AND comprometida<>1 and frecuencia=10";
    $result=query($query,$conexion);
    //ciclo para mostrar los datos
    while ($row = fetch_array($result))
    {       
...
?>
    ...
    <option>Seleccione una <?echo $termino?></option>
    <?php
    $query="SELECT codnom,descrip,codtip FROM nom_nominas_pago WHERE codtip='".$_SESSION['codigo_nomina']."'  AND contabilizada<>1 and frecuencia=10";
    $result=query($query,$conexion);
    //ciclo para mostrar los datos
    while ($row = fetch_array($result))
    {       
