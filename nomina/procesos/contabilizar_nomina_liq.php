...
//{
	...
	$consulta="SELECT DISTINCT(mn.codcon), c.ctacon FROM nom_movimientos_nomina mn left join nomconceptos c on (c.codcon=mn.codcon) WHERE mn.codnom=$nomina AND mn.tipnom='$_SESSION[codigo_nomina]' AND mn.tipcon='A' AND mn.codcon<>4029 and mn.codcon>4010";
	$result_con=query($consulta,$conexion);
	
	
	while($fetch_con=fetch_array($result_con))
	{
		$consulta="SELECT SUM(monto) as suma FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom='$_SESSION[codigo_nomina]' AND codcon=".$fetch_con['codcon']." AND tipcon='A' ";
		...
		$consulta_cwcondco="insert into cwcondco (Numcom,Fecha,Cuenta,Referen,Tiporef,Descrip,Debito,Credito,Numlim,FechaD) values ('".$ultimo_comprobante."','".date("Y-m-d")."','".$fetch_con['ctacon']."','".$fetch_ger['codorg']."','NO','".$fetch_descon['descrip']."','".$fetch_suma['suma']."','','".$i."','".date("Y-m-d")."')";
...
$consulta="SELECT DISTINCT(codcon) FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom='$_SESSION[codigo_nomina]' AND tipcon='D' AND  codcon<>2508 ORDER BY tipcon";
