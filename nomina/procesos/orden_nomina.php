...
function disponibilidad(){
	$conexion=conexion();
	//SE BUSCA TODAS LAS ASIGNACIONES PARA ESTA NOMINA
	$consulta="SELECT codcon,sum(monto)  as monto,codnivel3 FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina] AND tipcon='A'  group by codnivel3 order by cedula";
	$result=query($consulta,$conexion);
	
	$valida=false;
	while(($verificar=fetch_array($result))){
		$consulta = "SELECT ctacon1 FROM nomconceptos WHERE codcon = '".$verificar['codcon']."' ";	
		$result12 = query($consulta,$conexion);
		$fetch12 = fetch_array($result12);
	
		$sector=substr($verificar[codnivel3],0,2);
		$programa=substr($verificar[codnivel3],2,2);
		$actividad=substr($verificar[codnivel3],4,2);

		$consulta="SELECT * FROM cwprepar WHERE Sector='$sector' AND Programa='$programa' AND Actividad='$actividad' AND Codigo='$fetch12[ctacon1]'";
		$result13=query($consulta,$conexion);
		$fetch13 = fetch_array($result13);

		

		if($fetch13['Dispo'] < $verificar['monto'] ){
			
			echo "<SCRIPT language=\"JavaScript\" type=\"text/javascript\">
			alert(\"La Partida Presupuestaria $fetch12[ctacon1] del Sector $sector, Programa $programa, Actividad $actividad No Tiene Disponibilidad Suficiente\")
			</SCRIPT>";
			$valida=true;
...
$consulta_contrato="select * from nomempresa";
$query_contrato=query($consulta_contrato,$conexion);
$fet_contrato=fetch_array($query_contrato);

if($fet_contrato[contratado]==1 && $_SESSION[codigo_nomina]==4){
	...
	//segunda validacion si esta la orden de contrato en el adminsitrativo

	$consulta2="SELECT codcon,codnivel3,cedula FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina] AND tipcon='A'  group by  cedula";
	$result2=query($consulta2,$conexion);
	$valida=false;
	while(($verificar=fetch_array($result2))){
		$PROVEE="select cod_proveedor from proveedores where rif='$verificar[cedula]'";
		$query=query($PROVEE,$conexion);
		
		if(num_rows($query)!=0){
			$proveedor=fetch_array($query);	
			$select="select * from ordenes where cod_provee=$proveedor[cod_proveedor]";
			$ord=query($select,$conexion);
			if(num_rows($ord)==0){
				$valida=true;
				echo "<SCRIPT language=\"JavaScript\" type=\"text/javascript\">
				alert(\"La Contratado N° cedula= $verificar[cedula]  NO TIENE COMPROMISO ASOCIADO \")
				</SCRIPT>";
			}
		}else{
			echo "<SCRIPT language=\"JavaScript\" type=\"text/javascript\">
			alert(\"La Contratado N° cedula= $verificar[cedula]  NO EXISTE \")
			</SCRIPT>";
			$valida=true;
	...
	if($valida==false){ //si hay disponibilidad presupuestaria
		...
		//SE CONSULTA EL CODIGO MAS ALTO DE LAS ordenes de pago 
		$consulta="SELECT MAX(numero_odp) as cod_ord FROM ordenes_pago";
		$result4=query($consulta,$conexion);
		$fetch4=fetch_array($result4);
	
		$codigo_ord=$fetch4[cod_ord]+1;
		$insert_pago="insert into ordenes_pago values ($codigo_ord,'INTEGRANTES DEL LISTADO ANEXO', $fetch[monto],$fetch[monto],'".date("Y-m-d")."',0,'0000-00-00','','',0,'Emitida',0,'0000-00-00','','',0,'','','','0000-00-00','0000-00-00',0,'Nomina Contratado', '','','','','','','','','','','','','','','','','',0,0,0,0,0,0,0,0,'0000-00-00','',0,0,0,0,0,0,$fetch[monto],0,0,0,'','','$_SESSION[nombre]','0000-00-00','',$fetch[monto],'','','',0,0,'',0,'0000-00-00','',0,0,'','')";
		$query=query($insert_pago,$conexion);
		
		$inser_detalle="insert into ordenpago_det values($codigo_ord,1,'0.00','','NOMINA POR PAGAR','".date('Y-m-d')."','101.00.01')";
		$query=query($inser_detalle,$conexion);
	
		
	
		//SE BUSCA TODAS LAS ASIGNACIONES PARA ESTA NOMINA
		$consulta="SELECT * FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina] AND tipcon='A' order by cedula";
		$result=query($consulta,$conexion);
		
		$total=num_rows($result);
		while($i<$total){
			$fetch_mov=fetch_array($result);
			//SE CONSULTA LA CUENTA PRESUPUESTARIA DEL CONCEPTO ACTUAL
			$consulta = "SELECT ctacon1 FROM nomconceptos WHERE codcon = '".$fetch_mov['codcon']."' ";	
			$result12 = query($consulta,$conexion);
			$fetch12 = fetch_array($result12);
	
			$sector=substr($fetch_mov[codnivel3],0,2);
			$programa=substr($fetch_mov[codnivel3],2,2);
			$actividad=substr($fetch_mov[codnivel3],4,2);
			...
			$consulta="insert into cwpreeje values('',$fetch13[RecNo],$fetch_mov[monto],'".date("Y-m-d")."',$codigo_ord,0,'0000-00-00',0,'$fetch12[ctacon1]',$sector,$programa,$actividad,'','$_SESSION[nombre]','X',$fetch_mov[cedula])";
			$query=query($consulta,$conexion);
			
			//modificar disponibilidad

			$montoActualizado=$fetch13['AcuCau']+$fetch_mov[monto];
			$dispon=$fetch13['Dispo']-$fetch_mov[monto];
			$consulta="update cwprepar set AcuCau=$montoActualizado, Dispo='$dispon' WHERE Sector='$sector' AND Programa='$programa' AND Actividad='$actividad' AND Codigo='$fetch12[ctacon1]'";
			$consulta=query($consulta,$conexion);
			
			
			$PROVEE="select cod_proveedor from proveedores where rif='$fetch_mov[cedula]'";
			$query=query($PROVEE,$conexion);
			$proveedor=fetch_array($query);	
			
			$select="select * from ordenes where cod_provee=$proveedor[cod_proveedor]";
			$ord=query($select,$conexion);
			$ordenes=fetch_array($ord);
			$saldo=$ordenes[saldo]-$fetch_mov[monto];
			
			$consulta="update ordenes set saldo=$saldo where codigo=$ordenes[codigo]";
			$consulta=query($consulta,$conexion);
			$i++;
