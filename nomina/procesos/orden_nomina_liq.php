...
$consulta="SELECT cedula FROM nom_movimientos_nomina WHERE codnom='$nomina' and tipnom='$tipnom'";
$resultced=query($consulta,$conexion);
$fetchced=fetch_array($resultced);

$consulta="SELECT cod_proveedor FROM proveedores WHERE rif='".'V'.$fetchced['cedula']."'";
$resultprov=query($consulta,$conexion);
$fetchprov=fetch_array($resultprov);

$consulta="SELECT ccosto, unidad FROM nomempresa";
$resultma=query($consulta,$conexion);
$fetchma=fetch_array($resultma);


//SE CALCULA EL MONTO TOTAL DE LAS ASIGNACIONES PARA ESTA NOMINA - LIQ
$consulta="SELECT SUM(monto) as monto FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina] AND tipcon='A' and codcon<>'4029' and codcon>4010";
$result=query($consulta,$conexion);
$fetch=fetch_array($result);

//SE CALCULA EL MONTO TOTAL DE LAS DEDUCCIONES PARA ESTA NOMINA - LIQ
$consulta="SELECT SUM(monto) as monto FROM nom_movimientos_nomina WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina] AND tipcon='D' and codcon<>'2508'";
$resultD=query($consulta,$conexion);
$fetchD=fetch_array($resultD);

$mtoliq=$fetch[monto]-$fetchD[monto];
...
$consulta="INSERT INTO requisiciones SET cod_requisicion=".($fetch4['cod_req']+1).", agregada_fecha='".date('Y-m-d')."', situacion='Adjudicada', unidad='".$fetchma['unidad']."', cod_centro='".$fetchma['ccosto']."', concepto=(SELECT descrip FROM nom_nominas_pago WHERE codnom=$nomina AND tipnom=$tipnom), fecha='".date('Y-m-d')."', tipo=4";
...
//SE CONSULTA LOS CONCEPTOS PARA TRAER LOS CODIGOS DE LOS MATERIALES E INSERTARLOS CON SUS MONTOS EN LA REQUISICION
$consulta="SELECT mn.codcon, nc.ccosto, ma.descripcion FROM nom_movimientos_nomina mn left join nomconceptos nc on (mn.codcon=nc.codcon) left join materiales ma on (ma.cod_material=nc.ccosto) WHERE mn.codnom=$nomina and mn.tipnom=$tipnom and mn.codcon<>'4029' and mn.codcon<>'2508' and mn.codcon>4010 and mn.tipcon='A'";
$resultmat=query($consulta,$conexion);
$i=1;
while($fetchmat=fetch_array($resultmat))
{
	//SE HACE LA INSERCION EN REQUISICIONES DETALLES
	$consulta="INSERT INTO requisiciones_det SET cod_requisicion_det=".($fetch6['cod_req_det']+$i).", cod_requisicion=".($fetch4['cod_req']+1).", cod_material='".$fetchmat['ccosto']."', descripcion='$fetchmat[descripcion]', cantidad=1, unidad='".$fetchma['unidad']."', cod_centro='".$fetchma['ccosto']."'";
	$result7=query($consulta,$conexion);
	$i+=1;
}

$consulta="SELECT mn.codcon, nc.ccosto, ma.descripcion FROM nom_movimientos_nomina mn left join nomconceptos nc on (mn.codcon=nc.codcon) left join materiales ma on (ma.cod_material=nc.ccosto) WHERE mn.codnom=$nomina and mn.tipnom=$tipnom and mn.codcon<>'2508' and mn.tipcon='D'";
$resultmat=query($consulta,$conexion);
...
$consulta="INSERT INTO ordenes SET codigo=".($fetch3['cod_ord']+1).", fecha='".date('Y-m-d')."', unidad='".$fetchma['unidad']."', centro_costo='".$fetchma['ccosto']."', monto_orden=$mtoliq, concepto=(SELECT descrip FROM nom_nominas_pago WHERE codnom=$nomina AND tipnom=$_SESSION[codigo_nomina]), tipo=4, imponible=$mtoliq, monto_iva=0, monto_excento=0, cod_provee='".$fetchprov['cod_proveedor']."', cod_produ=0, cod_requi=".($fetch4['cod_req']+1).", estado='Emitida', saldo=$mtoliq, codigo_ref=$codigo_ref, condicioncompra='Otros', tipomoneda='Bolivares (Bs.)'";
$result8=query($consulta,$conexion);

//SE CONSULTA LOS CONCEPTOS PARA TRAER LOS CODIGOS DE LOS MATERIALES E INSERTARLOS CON SUS MONTOS EN LA ORDEN
$consulta="SELECT mn.codcon, nc.ccosto, ma.descripcion, mn.monto FROM nom_movimientos_nomina mn left join nomconceptos nc on (mn.codcon=nc.codcon) left join materiales ma on (ma.cod_material=nc.ccosto) WHERE mn.codnom=$nomina and mn.tipnom=$tipnom and mn.codcon<>'4029' and mn.codcon>4010 and mn.tipcon='A'";
$resultmato=query($consulta,$conexion);
$i=1;
while($fetchmato=fetch_array($resultmato))
{
	//SE HACE LA INSERCION EN ORDENES DETALLES
	$consulta="INSERT INTO ordenes_detalles SET cod_ord=".($fetch3['cod_ord']+1).", cod_pro='".$fetchmato['ccosto']."', cantidad_pedida=1, cantidad_des=1, precio=$fetchmato[monto], iva=0, total=$fetchmato[monto], total_gen=$fetchmato[monto], cod_requisicion=".($fetch4['cod_req']+1).", cod_ord_ref=".$codigo_ref."";
	$result9=query($consulta,$conexion);
}

$consulta="SELECT mn.codcon, nc.ccosto, ma.descripcion, mn.monto FROM nom_movimientos_nomina mn left join nomconceptos nc on (mn.codcon=nc.codcon) left join materiales ma on (ma.cod_material=nc.ccosto) WHERE mn.codnom=$nomina and mn.tipnom=$tipnom and mn.codcon<>'2508' and mn.tipcon='D'";
$resultmato=query($consulta,$conexion);
