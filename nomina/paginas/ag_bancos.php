...
<?php
$campo_clave='cod_ban';
$documento_list='bancos.php';
$documento_edit='ag_bancos.php';
$nombre_modulo='Bancos';
?>
...
<?php 
	...
	if ($registro_id==0){ // Si el registro_id es 0 se va a agregar un registro nuevo
		...
		(cod_ban,cod_gban,des_ban,suc_ban,direccion,gerente,cuentacob,
		tipocuenta,markar,ee,textoinicial,textofinal,ctacon)
		values ('$codigo_nuevo','$_POST[cboGrupo]','$_POST[txtdescripcion]','$_POST[txtsucursal]',
		'$_POST[txtdireccion]','$_POST[txtgerente]','$_POST[txtcuenta]',
		'$_POST[optTipoCuenta]','0','0','$_POST[txtTextoInicial]','$_POST[txtTextoFinal]','$_POST[txtcuentacont]')";
	...
	if ($op_tp==2){					
			...
			cod_gban='$_POST[cboGrupo]',
			ctacon='$_POST[txtcuentacont]'
  ...
  <table width="780" height="125" border="0" class="row-br">
    ...
    <tr>
      <td width="489" height="86" class="ewTableAltRow"><table width="790" border="0" bordercolor="#0066FF">
        <tr bgcolor="#FFFFFF">
          <td height="23" bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">C&oacute;digo del Grupo:</font></td>
	 	...
	 	$query="select cod_gban,des_ban from nomgrupo_bancos";
		$result=sql_ejecutar($query);
		...
		// Opcion de modificar, se selecciona la situacion del registro a modificar		
  		if ($row[cod_gban]==$grupo){ ?>
              <option value="<?php echo $row[cod_gban];?>" selected > <?php echo $row[des_ban];?> </option>
              <?php 
		}
		else // opcion de agregar
		{ 
		   ?>
              <option value="<?php echo $row[cod_gban];?>"><?php echo $row[des_ban];?></option>
              <?php 
		} 
		}//fin del ciclo while
		?>
        ...
        <tr bgcolor="#FFFFFF">
          <td width="207" height="23" bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">C&oacute;digo del Banco:</font></td>
          <td width="573" bgcolor="#FFFFFF" class="ewTableAltRow" ><font size="2" face="Arial, Helvetica, sans-serif">
            <input name="txtcodigo" type="text" id="txtcodigo" disabled="disabled" style="width:100px"  event.returnValue = false;" value="<?php if ($registro_id!=0){ echo $codigo; }  ?>" maxlength="10">
          </font></td>
          </tr>
        ...
        <tr bgcolor="#FFFFFF">
          ...
          <td bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
            <input name="txtsucursal" type="text" id="txtsucursal" style="width:200px" value="<?php if ($registro_id!=0){ echo $sucursal; }  ?>" >
          </font></td>
        </tr>
        <tr bgcolor="#FFFFFF">
          <td height="24" bgcolor="#FFFFFF" class="ewTableAltRow">Direcci&oacute;n : </td>
          <td bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
            <input name="txtdireccion" type="text" id="txtdireccion"  size="100"  value="<?php if ($registro_id!=0){ echo $direccion; }  ?>">
          </font></td>
        </tr>
        ...
        <tr bgcolor="#FFFFFF">
          ...
          <td rowspan="4" bgcolor="#FFFFFF" class="ewTableAltRow"><textarea name="txtTextoFinal" id="txtTextoFinal" style=" height:100px;width:500px"><?php echo $texto_final; ?></textarea>          </td>
        </tr>
        <tr bgcolor="#FFFFFF">
        ...
        <tr bgcolor="#FFFFFF">
          <td height="26" bgcolor="#FFFFFF" class="ewTableAltRow">Cuenta Contable: </td>
          <td bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
	<select name="txtcuentacont" id="txtcuentacont" >
	<option value="<?php if ($registro_id!=''){ echo $cuenta_cont; } ?>"><?php if ($registro_id!=''){ echo $cuenta_cont; }else echo "Seleccione cuenta"; ?></option>
	<?php 
	$consulta="SELECT Cuenta, Descrip FROM cwconcue WHERE Nivel=7 AND Cuenta LIKE '1.%'";
	$resultado333=sql_ejecutar($consulta);
	while($fetch = fetch_array($resultado333))
	{
	?>
	<OPTION title="<? echo $fetch['Descrip']?>" value="<?php echo $fetch['Cuenta'];?>"><?php echo $fetch['Cuenta'];?></OPTION>
	<?php
	}
	?>
	</select>
