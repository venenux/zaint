...
function enviar(op,id,nomina,codtip){
	...
	if (op==8){		// Generar Nómina
		document.frmPrincipal.registro_id.value=id;
		document.frmPrincipal.codigo_nomina.value=nomina;
		AbrirVentana('movimientos_agregar_masivo.php?nomina='+id,200,580,1);
	}
	if(op==9)
	{		// Generar Nómina
		document.frmPrincipal.registro_id.value=id;
		document.frmPrincipal.codigo_nomina.value=nomina;
		AbrirVentana('movimientos_agregar_masivo_nom.php?nomina='+id,500,580,1);
	}
	if(op==10)
	{		// Generar Nómina
		document.frmPrincipal.registro_id.value=id;
		document.frmPrincipal.codigo_nomina.value=nomina;
		AbrirVentana('movimientos_agregar_masivo_nom_desc.php?nomina='+id,300,710,1);
	}
}
</script>
...
{	
	...
	$strsql= "select COUNT(*) from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia!='6' and frecuencia!='8' and frecuencia!='10'";
	$result =sql_ejecutar($strsql);	
	$fila = fetch_array($result);
	
	$num_total_registros = $fila[0];
	
	$strsql= "select * from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia!='6' and frecuencia!='8' and frecuencia!='10' order by $campo_1,$campo_2 ";
	$result =sql_ejecutar($strsql);		
	$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
$conexion=conexion();
...
$consulta_max="select max(codnom) as maximo from nom_nominas_pago where tipnom='".$_SESSION['codigo_nomina']."' and (frecuencia<>6 and frecuencia<>10)";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td><div align="right"><?php btn('add','ag_nomina_pago.php') ?></div></td>
<td><div align="right"> 
...
<?if($fila["status"]=="A"){?>
	...
	<td><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(8); ?>,<?php echo($fila[$campo_1]); ?>,<?php echo $_SESSION['codigo_nomina']?>,0);"><img src="img_sis/ico_add.gif" title="Agregar variantes" width="15" height="15" border="0" align="absmiddle" ></a></font></td>
	<td><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(9); ?>,<?php echo($fila[$campo_1]); ?>,<?php echo $_SESSION['codigo_nomina']?>,0);"><img src="img_sis/ico_est2.gif" title="Agregar variantes" width="15" height="15" border="0" align="absmiddle" ></a></font></td>
	<td><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(10); ?>,<?php echo($fila[$campo_1]); ?>,<?php echo $_SESSION['codigo_nomina']?>,0);"><img src="img_sis/ico_est4.gif" title="Agregar variantes" width="15" height="15" border="0" align="absmiddle" ></a></font></td>
