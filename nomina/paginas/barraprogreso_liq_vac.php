...
<form id="frmPrincipal" name="frmPrincipal" method="post" action="">
  ...
  <?php
	...
	if ($op==1)
	{
		...
		$query="delete from nom_movimientos_nomina where tipnom='".$_SESSION['codigo_nomina']."' and codnom='$registro_id' and ficha='".$_GET['ficha']."'";			
		...
		where cf.codfre='".$fila_nom['frecuencia']."' and pe.tipnom = '".$_SESSION['codigo_nomina']."' and ct.codtip = '".$_SESSION['codigo_nomina']."' and cs.estado = pe.estado and pe.ficha = '".$_GET['ficha']."' group by pe.apenom,pe.ficha,c.formula,c.codcon,cs.estado order by c.tipcon, c.codcon";
		$result=sql_ejecutar($query);
			...
			if ($fila['formula']!='')
			{
				...
				if ($fila[contractual]==1){
					...
					if ($entrar==1)
					{
						...
						(codnom,codcon,ficha,monto,cedula,tipcon,unidad,valor,descrip,tipnom,contractual,impdet) values ('$registro_id','".$fila[codcon]."','".$fila[ficha]."','$MONTO','$CEDULA','".$fila[tipcon]."','".$fila[unidad]."','".$REF."','".$fila['descrip']."','".$_SESSION['codigo_nomina']."','".$fila[contractual]."','".$fila['impdet']."')";
	  ...
	  <?php	
		...
		opener.location.href = "movimientos_nomina_liquidaciones.php?codigo_nomina="+document.frmPrincipal.codnomm.value+"&codt="+document.frmPrincipal.codtt.value+"&bandera="+1+"&pagina="+document.frmPrincipal.pagg.value
