...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function calcular_antiguedad(fecha){
                var egre=document.getElementById('txtegreso')
                if(egre!=null){
                    egre=egre.value
                    var array_fecha = egre.split("/")
                    var array_fecha2 = fecha.split("/")
                    if (array_fecha2.length!=3)
                        return "La fecha introducida es incorrecta"
                    var ano
                    ano = parseInt(array_fecha2[2]);
                    if (isNaN(ano))
                        return "El año es incorrecto"

                    var mes
                    mes = parseInt(array_fecha2[1]);
                    if (isNaN(mes))
                        return "El mes es incorrecto"

                    var dia
                    dia = parseInt(array_fecha2[0]);
                    if (isNaN(dia))
                    ...
                    edad=array_fecha[2]- ano - 1; //-1 porque no se si ha cumplido años ya este año
                    var meses

                    if (array_fecha[1] + 1 - mes > 0 ){
                        meses=array_fecha[1] - mes

                        edad= edad+1
                    }
                    if(array_fecha[1]  + 1 - mes < 0)
                    {
                        meses=array_fecha[1]  + 1 - mes+12
                    }
                    if(array_fecha[1]  + 1 - mes == 0){
                        meses=0
                    }
                    ...
                    if (array_fecha[0]  - dia >= 0){
                        dias=array_fecha[0]  - dia+1
                        if(meses==0){
                            edad=edad + 1
                        }
                    }else{
                        dias=array_fecha[0] -dia+30
                    }
            ...
            function Verificar(estado){
                ...
                var ficha=document.frmAgregarIntegrantes.txtficha.value
                location.href="ed_maestro_integrantes.php?cedula="+valor+"&estado="+estado+"&ficha="+ficha
            }
        </script>
    </head>
    <body>
        ...
        if (isset($_POST['guardar'])) {
            if ($_POST['txtficha'] != "" && $_POST['txtcedula'] != "" && $_POST['txtnombres'] != "" && $_POST['txtapellidos'] != "") {
                ...
                if ($archivo != "") {
                    ...
                    if (copy($_FILES['mifichero']['tmp_name'], "fotos/" . $nombre_archivo1)) {
                ...
                if ($_POST['cbosituacion'] == 'Egresado') {
                    $anexo = ", fecharetiro='" . fecha_sql($_POST['txtegreso']) . "'";
                }
	...
	motivo_retiro='" . $_POST['motivo'] . "',
	inicio_periodo='" . fecha_sql($_POST['fecha_inicio_contrato']) . "',
	fin_periodo='" . fecha_sql($_POST['fecha_fin_contrato']) . "'" . $anexo . ", antiguedadap='" . $_POST['antiguedadap'] . "', paso='$_POST[txtpaso]' WHERE cedula='" . $_POST['txtcedula'] . "' and ficha='" . $_POST['txtficha'] . "'";
        ...
        $cedula = $_GET['cedula'];
        $fichass = $_GET['ficha'];
        $consulta = "select * from nompersonal where cedula='" . $cedula . "' and ficha=$fichass";
        ...
        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="frmAgregarIntegrantes" id="frmAgregarIntegrantes" enctype="multipart/form-data">
            ...
            <table align="left" width="700" border="0">
                <tbody>
                    <tr><TD colspan="3" class="tb-tit" align="right"><?
        if ($bandera == 1)
            echo btn("back", "maestro_personal.php?bandera=1"); else
            echo btn("back", "maestro_personal.php");
        ?></TD></tr>
            ...
            <table align="left" width="700" border="0" class="">
                ...
                <tr>
                    ...
                    <td>
                        <font size="2" face="Arial, Helvetica, sans-serif"><input name="txtnombres" type="text" id="txtnombres" style="width:200px" value="<?php echo $fila['nombres']; ?>" maxlength="30">
                        </font>
                    </td>
                </tr>
                <tr>
...
//ciclo para mostrar los datos
    ...
    if ($fila['estado'] == 'Egresado') { ?>
                                <!--egresado -->
                                <div>
                                    <font size="2" face="Arial, Helvetica, sans-serif"> Fecha de renuncia: </font>
                                    <input  name="txtegreso" type="text" id="txtegreso" style="width:100px" value="<?php echo fecha($fila ['fecharetiro']) ?>" maxlength="60">
                                    <!-- onblur="javascript:actualizar('txtJuvilacion','fila_edad');"-->
                                    <input name="d_fechaegreso" type="image" id="d_fechaegreso" src="../lib/jscalendar/cal.gif" />
                                    <script type="text/javascript">Calendar.setup({inputField:"txtegreso",ifFormat:"%d/%m/%Y",button:"d_fechaegreso"});</script>
                                    <font size="2" face="Arial, Helvetica, sans-serif"> Motivo: </font>
                                    <select id='motivo' name='motivo'>
                                        ...
                                        <option value='Culminacion de contrato'>Culminacion de contrato</option>
    ...
    <?php }
                ...
                <tr>
                    <td><font size="2" face="Arial, Helvetica, sans-serif">Fecha de Inclusi&oacute;n en Caja de Ahorro:</font></td>
                    <td>
                        <font size="2" face="Arial, Helvetica, sans-serif">
                        <input name="txtFechaInclusion" type="text" id="txtFechaInclusion" style="width:100px" value="<?php echo fecha($fila['dfecing']); ?>" maxlength="60"/>
...
<? if ($termino != "Planilla") { ?><input name="txtcuentaaux" type="text" id="txtcuentaaux" style="width:200px" value="<? echo $fila['cuentalph'] ?>" maxlength="20"><? } ?>
                ...
                for ($i = 1; $i <= 15; $i++) {
                    ?>
                                    <option value="<? echo $i; ?>" <? if ($i == $fila['paso'])
                    echo "selected='true'" ?>><? echo $i; ?></option>
                    <?
                }
                ?>
                        </select></td>
