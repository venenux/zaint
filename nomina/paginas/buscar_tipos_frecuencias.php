...
if ($op==1) //Se presiono el boton de Aceptar
	...
	//$strsql1="delete from nomconceptos_frecuencias where codcon = $codigo_con";
	//$result1=sql_ejecutar($strsql1);			
	//$strsql1="";
	
	foreach($_POST['chkCodigo'] as $key => $value)	{	
		$strsql1="select * from nomconceptos_frecuencias where codcon='$codigo_con' and codfre='$value'";
		$result1=sql_ejecutar($strsql1);			
		if(mysql_num_rows($result1)==0){	
			$strsql1="insert into nomconceptos_frecuencias values ($codigo_con,$value,0)";
			$result1=sql_ejecutar($strsql1);			
			$strsql1="";
		}
	}
...
{	
	...
	$strsql= "select * from nomfrecuencias order by descrip,codfre LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
?>
    ...
    <tr>
      <td class="row-br"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
			...
			<strong>
			<font color="#000066">
			Seleccionar Tipos de Frecuencia </font></strong></td>
  ...
  <table width="95%" border="0" class="ewTableHeader">
    <tr class="">
      ...
      <td width="17"><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
        <?php btn('show_all',"buscar_tipos_frecuencias.php?cmd=reset&txtcodigo=$codigo_con") ?>
      </font></td>
  ...
  <table width="95%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
    ...
    <tr> 
      ...
      <td height="20" bgcolor="#DFDFDF" class="ewTableRow"> <div align="left" class="row-even"><font size="2" face="Arial, Helvetica, sans-serif">	
          <label>
		  <?php
			$query="select * from nomvis_conceptos_frecuencia where codcon=$codigo_con and codfre=$fila[codfre]";					
			...
			{$bActivar=1;}
		  ?>
          <input type="checkbox" name="chkCodigo[]" id="chkCodigo[]" value="<?php echo $fila[codfre]; ?>"<?php
		  if ($bActivar==1)
		  {
		  ?>
		   checked="checked"
	...
	?>
          ...
          <table border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <?php } else { ?>
              <td><a href="buscar_tipos_frecuencias.php?pagina=1&criterio=<?php $criterio;?>&txtcodigo=<?php echo $codigo_con; ?>"><img src="images/first.gif" alt="Primera" width="16" height="16" border="0"></a></td>
              <?php } ?>
              <!--previous page button-->
              <?php if ($PrevStart == $inicio) { ?>
              <td><img src="images/prevdisab.gif" alt="Anterior" width="16" height="16" border="0"></td>
              <?php } else { ?>
              <td><a href="buscar_tipos_frecuencias.php?pagina=<?php echo $pagina-1; ?>&criterio=<?php $criterio;?>&txtcodigo=<?php echo $codigo_con; ?>"><img src="images/prev.gif" alt="Anterior" width="16" height="16" border="0"></a></td>
              <?php } ?>
              <!--current page number-->
              ...
              <?php } else { ?>
              <td><a href="buscar_tipos_frecuencias.php?pagina=<?php echo $pagina+1; ?>&criterio=<?php $criterio;?>&txtcodigo=<?php echo $codigo_con; ?>"><img src="images/next.gif" alt="Siguiente" width="16" height="16" border="0"></a></td>
              <?php  } ?>
              <!--last page button-->
              <?php if ($NextStart == $inicio) { ?>
              <td><img src="images/lastdisab.gif" alt="Ultima" width="16" height="16" border="0"></td>
              <?php } else {?>
              <td><a href="buscar_tipos_frecuencias.php?pagina=<?php  echo $LastStart; ?>&criterio=<?php $criterio;?>&txtcodigo=<?php echo $codigo_con; ?>
