...
$url="movimientos_nomina_pago_agregar";
	...
	if($SITUACION!="Inactivo")
	{
		foreach($_POST['seleccion'] as $valor)
		{
			...
			if(num_rows($resultado_mov)==0)
			{
				...
				if($entrar==1)
				{
				$consulta="insert into nom_movimientos_nomina (codnom, codcon,ficha,mes,anio,tipcon,valor,monto,cedula,unidad,descrip,codnivel1,codnivel2,codnivel3,codnivel4,codnivel5,codnivel6,codnivel7,tipnom,contractual) values ('".$_POST['nomina']."', '".$concepto[$valor]."','".$_POST['ficha']."','".$fila_nom['mes']."','".$fila_nom['anio']."','".$tipoconcepto[$valor]."','".$REF."','".$MONTO."','$CEDULA','".$unidad[$valor]."','".$descripcion[$valor]."','$NIVEL1','$NIVEL2','$NIVEL3','$NIVEL4','$NIVEL5','$NIVEL6','$NIVEL7','".$_SESSION['codigo_nomina']."','$fila[contractual]')";
...
nomconceptos_tiponomina.codtip='".$_SESSION['codigo_nomina']."' and nomconceptos_frecuencias.codfre='".$fila_nom['frecuencia']."' group by nomconceptos.tipcon, nomconceptos.codcon, nomconceptos.descrip order by nomconceptos.tipcon,nomconceptos.descrip";
}
