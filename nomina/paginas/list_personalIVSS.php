...
function AbrirRetiro(id,nom)
{
location.href='../fpdf/1403pdf.php?registro_id='+id+'&tipN='+nom

}
function AbrirRegistro(id,nom)
{
location.href='../fpdf/rpt_ivss_registropdf.php?registro_id='+id+'&tipN='+nom
...
elseif ($cadena <> "") 		// Condicion para filtrado
	{   
		...
		$strsql="select COUNT(*) from nomvis_integrantes where estado='Nuevo' or estado='Egresado'";		
		...
		$strsql="select * from nomvis_integrantes estado='Nuevo' or estado='Egresado'";
		$strsql=filtrado($criterio,$cadena,$strsql,"cedula","nombres","apellidos","ficha","descrip",
		'','','','','',"'".$_SESSION[nomina]."'",'descrip');
...
else					// No se filtra y se muestran todos los datos
{
	...
	$strsql= "select COUNT(*) from nomvis_integrantes where descrip='".$_SESSION['nomina']."'and (estado='Nuevo' or estado='Egresado')";
	$result =sql_ejecutar($strsql);	
	$fila = mysql_fetch_array($result);	
	$num_total_registros = $fila[0];
	
	$strsql= "select * from nomvis_integrantes where descrip='".$_SESSION['nomina']."' and(estado='Nuevo' or estado='Egresado') order by apellidos,nombres LIMIT $TAMANO_PAGINA  OFFSET $limit";
...
<input name="optOpcion" type="radio" id="Contenga"  value="Contenga"   checked="true"
  ...
  <table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
	...
	<?php if ($fila[estado]=='Egresado') { ?>
		javascript:AbrirRetiro('<?php echo $fila[ficha]; ?>','<?php echo $_SESSION['codigo_nomina'];?>');
	 <?php } if ($fila[estado]=='Nuevo') { ?> 
		javascript:AbrirRegistro('<?php echo $fila[ficha]; ?>','<?php echo $_SESSION['codigo_nomina'];?>'); 
	<?php }
 ?>
