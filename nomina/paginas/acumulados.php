...
function enviar(op,id){
	...
	if (op==1){		// Opcion de Agregar
		...
		document.frmPrincipal.action="ag_acumulados.php";
...
<link href="lib/spry/SpryValidationTextField.css" rel="stylesheet" type="text/css">
...
<link href="../../asys/fckeditor/editor/skins/silver/fck_contextmenu.css" rel="stylesheet" type="text/css"></head>
</html>
<html>
<head>
</head>
...
if ($op==3) {//Se presiono el boton de Eliminar			
	$query="delete from nomacumulados where cod_tac='$registro_id'";			
...
elseif ($cadena <> ""){ // Condicion para filtrado
		...
		$strsql="select COUNT(*) from nomacumulados";
		$strsql=filtrado($criterio,$cadena,$strsql,"cod_tac","des_tac");		
		$result =sql_ejecutar($strsql);			
		$fila = mysql_fetch_array($result);	
		$num_total_registros = $fila[0];	
	
		$strsql="select * from nomacumulados";
		$strsql=filtrado($criterio,$cadena,$strsql,"cod_tac","des_tac");		
	...
	$strsql= "select * from nomacumulados order by cod_tac,des_tac LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
?>
...
<form action="" method="post" name="frmPrincipal" id="frmPrincipal">
  ...
  <table width="100%" border="0" class="ewTableHeader">
    <tr class="">
      ...
      <td width="17"><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
        <?php btn('show_all','acumulados.php?cmd=reset') ?>
      </font></td>
...
<label>
  ...
  <table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
    ...
    <tr bgcolor="#CCCCCC" class="ewTableHeader"> 
    	<td width="23%" height="21" align="right" class="phpmakerlist"> 
			<div align="left" class="tb-head">
	  <font size="2" face="Arial, Helvetica, sans-serif"> Tipo  </font></div>	  </td>
		
        <td width="71%" class="phpmakerlist"><div align="left"><font size="2" face="Arial, Helvetica, sans-serif">Descripci&oacute;n</font></div></td>
    ...
    <tr> 
      ...
      <td height="20" bgcolor="#DFDFDF" class="ewTableRow"> <div align="left" class="row-even"><font size="2" face="Arial, Helvetica, sans-serif">	
          <?php 
		  echo $fila[cod_tac]; 	// codigo de la categoria
		  ?>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="left"><font size="2" face="Arial, Helvetica, sans-serif">
        <?php 
	  		echo $fila[des_tac];  	// salario de la categoria
	  	?>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(2); ?>,'<?php echo($fila[cod_tac]); ?>');"><img src="img_sis/ico_edit.gif" alt="Modificar el Registro Actual" width="16" height="16" border="0" align="absmiddle"></a>
        <label></label>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(3); ?>,'<?php echo($fila[cod_tac]); ?>');"><img src="../imagenes/delete.gif" alt="Eliminar el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a></font></div></td>
    </tr>
    <?php   
  	}//fin del ciclo while
  ...
  <table width="100%" height="31" class="tb-footer">
	...
	?>
        ...
        <tr>
          ...
          <?php } else { ?>
          <td><a href="acumulados.php?pagina=1&criterio=<?php $criterio;?>"><img src="images/first.gif" alt="Primera" width="16" height="16" border="0"></a></td>
          <?php } ?>
          <!--previous page button-->
          <?php if ($PrevStart == $inicio) { ?>
          <td><img src="images/prevdisab.gif" alt="Anterior" width="16" height="16" border="0"></td>
          <?php } else { ?>
          <td><a href="acumulados.php?pagina=<?php echo $pagina-1; ?>&criterio=<?php $criterio;?>"><img src="images/prev.gif" alt="Anterior" width="16" height="16" border="0"></a></td>
          <?php } ?>
          <!--current page number-->
          ...
          <?php } else { ?>
          <td><a href="acumulados.php?pagina=<?php echo $pagina+1; ?>&criterio=<?php $criterio;?>"><img src="images/next.gif" alt="Siguiente" width="16" height="16" border="0"></a></td>
          <?php  } ?>
          <!--last page button-->
          <?php if ($NextStart == $inicio) { ?>
          <td><img src="images/lastdisab.gif" alt="Ultima" width="16" height="16" border="0"></td>
          <?php } else {?>
          <td><a href="acumulados.php?pagina=<?php  echo $LastStart; ?>&criterio=<?php $criterio;?>
