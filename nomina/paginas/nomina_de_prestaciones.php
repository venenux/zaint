...
/////////////////////////////
$nombre_modulo=$termino.' de Prestaciones';
...
$campo_9='';
$campo_10='';
$documento_list='nomina_de_prestaciones.php';
...
function enviar(op,id,nomina,codtip){
	...
	if (op==5){	// Movimiento de Nómina
		parent.cont.location.href="movimientos_nomina_pago.php?codigo_nomina="+id+"&codt="+codtip+"&prestaciones="+1
		...
		if (confirm("Esta seguro que desea cerrar esta <?echo $termino?> ?"))
		{
			...
			cerrar_nomina.onreadystatechange=function() 
			{
				if (cerrar_nomina.readyState==4)
				{
					...
					parent.cont.location.href="nomina_de_prestaciones.php"
				}
			}
...
{	
	$strsql= "select COUNT(*) from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia='6'";
	$result =sql_ejecutar($strsql);	
	$fila = fetch_array($result);
	
	$num_total_registros = $fila[0];
	
	$strsql= "select * from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia='6' order by $campo_1,$campo_2 ";
	$result =sql_ejecutar($strsql);		
	$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
$conexion=conexion();
...
$consulta_max="select max(codnom) as maximo from nom_nominas_pago where tipnom='".$_SESSION['codigo_nomina']."' and frecuencia=6";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td><div align="right"><?php btn('add','ag_nomina_prestaciones.php') ?></div></td>
<td><div align="right"> 
