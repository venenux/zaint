...
<form id="frmPrincipal" name="frmPrincipal" method="post" action="">
  ...
  <?php
	...
	if ($op==1)
	{
		...
		$query="delete from nom_movimientos_nomina where tipnom='".$_SESSION['codigo_nomina']."' and codnom='$registro_id' and contractual='1'";
		$result3=sql_ejecutar($query);
		// PARA ACTIVAR EL BONO POR RAZON DE SERVICIO EN EL CSB
		unset($result3);

		$query_historico="delete from nom_movimientos_historico where tipnom='".$_SESSION['codigo_nomina']."' and codnom='$registro_id'";
...
// 		unset($resultado_per);
		...
		where cf.codfre='".$fila_nom['frecuencia']."' and pe.tipnom = '".$_SESSION['codigo_nomina']."' and ct.codtip = '".$_SESSION['codigo_nomina']."' and cs.estado = pe.estado and c.contractual='1'
...
// 		$PRS=$bandera;
		...
		while ($fila = fetch_array($result2))
		{
			...
			if($HISTORICO=='' || $HISTORICO!=$CEDULA){
				$HISTORICO=$CEDULA;
				$insert="insert into nom_movimientos_historico values ('','$registro_id','".$_SESSION['codigo_nomina']."','$NIVEL1','$NIVEL2','$NIVEL3','$NIVEL4','$NIVEL5','$NIVEL6','$NIVEL7','$FICHA','$SUELDO','$CODCARGO','$SITUACIONPER','$CEDULA')";
			...
			if ($fila['formula']!='')
			{
				...
				if ($fila[contractual]==1){
					...
					if ($entrar==1)
					{
						...
						(codnom,codcon,ficha,mes,anio,monto,cedula,tipcon,unidad,valor,descrip,codnivel1,codnivel2,codnivel3,codnivel4,codnivel5,codnivel6,codnivel7,tipnom,contractual,saldopre) values ('$registro_id','".$fila[codcon]."','".$fila[ficha]."','".$fila_nom[mes]."','".$fila_nom[anio]."','$MONTO','$CEDULA','".$fila[tipcon]."','".$fila[unidad]."','".$REF."','".$fila['descrip']."','$fila[codnivel1]','$fila[codnivel2]','$fila[codnivel3]','$fila[codnivel4]','$fila[codnivel5]','$fila[codnivel6]','$fila[codnivel7]','".$_SESSION['codigo_nomina']."','".$fila['contractual']."','$SALDOPRE')";
