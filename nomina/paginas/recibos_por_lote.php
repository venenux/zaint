...
<?
	...
	$encabezado=encabezado('','','','','../imagenes/SiSalud.jpg'.$var_imagen_izq,'../imagenes/dot.JPG'.$var_imagen_der);
	//cerrar_conexion($Conn);
	
 $nomina_id=$_GET['nomina_id'];
 $codtp=$_GET['codt'];

$consulta3 = "SELECT periodo_ini, periodo_fin FROM nom_nominas_pago WHERE codnom = '".$nomina_id."' AND codtip = '".$codtp."'";
...
$query="select * from nomvis_per_movimiento where codnom='".$nomina_id."' and tipnom='".$codtp."' ";		
...
<?
	$contador=1;
while($fila=mysql_fetch_array($result_lote))
{
  ...
  <table width="754" height="106" border="0">
    <tr>
      <td>
        ...
        <table width="754" style="border-bottom-style : solid; border-left-style : solid; border-right-style : solid; border-top-style : solid;" border="1">
          <tr>
...
<table width="754" border="1">
	...
	<td align="center" ><strong>Banco/Cuenta:</strong></td>
          ...
          <tr>
            <td align="center" ><strong><?echo $_SESSION['nomina'];?> -- DESDE:&nbsp;<?echo fecha($fetch5['periodo_ini'])?> HASTA:&nbsp;<?echo fecha($fetch5['periodo_fin'])?></strong></td>
            <td align="center" ><strong><?php 
	...
	<br>
		...
		<td width="87" colspan="2"><div align="center"><strong>Patronales</strong></div></td>
			...
			where pe.ficha = '$registro_id' and mn.codnom= '".$nomina_id."' and mn.tipnom='".$codtp."'
