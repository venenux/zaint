...
function enviar(op,id){
	...
	if (op==1){		// Opcion de Agregar
		...
		document.frmAgregar.action="ag_instruccion.php";
	...
	if (op==2){	 	// Opcion de Modificar
		...
		document.frmCargos.action="ag_instruccion.php";
...
if ($op==3) //Se presiono el boton de Eliminar
	...
	$query="delete from nominstruccion where codigo=$registro_id";	
...
elseif ($cadena <> "") 		// Condicion para filtrado
	{   
		...
		$strsql="select COUNT(*) from nominstruccion";
		...
		$strsql="select * from nominstruccion";
		$strsql=filtrado($criterio,$cadena,$strsql,"codigo","descripcion");		
...
else					// No se filtra y se muestran todos los datos
{
	...
	$strsql= "select * from nominstruccion order by codigo,descripcion LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
?>
...
<form action="" method="post" name="frmCargos" id="frmCargos">
    ...
    <tr>
      <td class="row-br"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
			...
			<strong>
			<font color="#000066">
			Grado de instrucci&oacute;n </font></strong></td>
            ...
            <td width="2%" align="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td>
				    <div align="right">
				      <?php btn('add','ag_instruccion.php') ?>				        
		            </div></td>
                </tr>
            </table></td>
          </tr>
  ...
  <table width="100%" border="0" class="ewTableHeader">
    <tr class="">
      ...
      <td width="17"><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
        <?php btn('show_all','instruccion.php?cmd=reset') ?>
      </font></td>
  ...
  <table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
    ...
    <tr bgcolor="#CCCCCC" class="ewTableHeader"> 
      ...
      <td width="72%" class="phpmakerlist">
	  	<div align="left">
	  		<font size="2" face="Arial, Helvetica, sans-serif">
	  Descripci&oacute;n del grado de instrucci&oacute;n	  			  </font></div>	  </td>
  ...
  <table width="100%" height="31" class="tb-footer">
	...
	?>
        ...
        <tr>
          ...
          <?php } else { ?>
          <td><a href="instruccion.php?pagina=1&criterio=<?php $criterio;?>"><img src="images/first.gif" alt="Primera" width="16" height="16" border="0"></a></td>
          <?php } ?>
          <!--previous page button-->
          <?php if ($PrevStart == $inicio) { ?>
          <td><img src="images/prevdisab.gif" alt="Anterior" width="16" height="16" border="0"></td>
          <?php } else { ?>
          <td><a href="instruccion.php?pagina=<?php echo $pagina-1; ?>&criterio=<?php $criterio;?>"><img src="images/prev.gif" alt="Anterior" width="16" height="16" border="0"></a></td>
          <?php } ?>
          <!--current page number-->
          ...
          <?php } else { ?>
          <td><a href="instruccion.php?pagina=<?php echo $pagina+1; ?>&criterio=<?php $criterio;?>"><img src="images/next.gif" alt="Siguiente" width="16" height="16" border="0"></a></td>
          <?php  } ?>
          <!--last page button-->
          <?php if ($NextStart == $inicio) { ?>
          <td><img src="images/lastdisab.gif" alt="Ultima" width="16" height="16" border="0"></td>
          <?php } else {?>
          <td><a href="instruccion.php?pagina=<?php  echo $LastStart; ?>&criterio=<?php $criterio;?>
