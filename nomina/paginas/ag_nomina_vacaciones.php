...
/*function fecha_sql($value) { // fecha de DD/MM/YYYY a YYYYY/MM/DD
 return substr($value,6,4) ."/". substr($value,3,2) ."/". substr($value,0,2);
	...
	{			
		...
		//exit(0);
		$query="insert into nom_nominas_pago (codnom,descrip,fechapago,periodo_ini,periodo_fin,anio,mes,frecuencia,periodo,status,codtip,tipnom)
		...
		".date("Y").",".$mes.",'$_POST[cboFrecuencia]','$_POST[sel_periodo]','A','".$_SESSION['codigo_nomina']."','".$_SESSION['codigo_nomina']."')";
		
		$result=sql_ejecutar($query);
		
		activar_pagina("nomina_de_vacaciones.php");
		}
	}
	...
	<table width="790" border="0" bordercolor="#0066FF">
        ...
        <tr valign="middle" bgcolor="#FFFFFF">
          ...
          <td colspan="5" valign="middle" bgcolor="#FFFFFF" class="ewTableAltRow" ><font size="2" face="Arial, Helvetica, sans-serif">
            <select onBlur="ActualizarNombre('<?php echo ($_SESSION[nomina]) ?>');" onchange="javascript:periodo()" name="<?php echo $row[codfre];?>cboFrecuencia" id="cboFrecuencia" style="width:200px">
              <?php
	 	$query="select codfre,descrip from nomfrecuencias where codfre=8";
		$result=sql_ejecutar($query);
        ...
        <tr>
	<td height="24" bgcolor="#FFFFFF" class="ewTableAltRow">Periodo: </td>
	<td><div id="periodo">
	<SELECT name="sel_periodo" disabled="true" id="sel_periodo" onchange="javascript:cargar_fecha()">
	<option value="0">Seleccione un periodo</option>
        ...
        <tr bgcolor="#FFFFFF">
          ...
          <td width="141" bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
            ...
            <input onChange="ActualizarNombre('<?php echo ($_SESSION[nomina]) ?>');" name="txtfechainicio" type="text" id="txtfechainicio" style="width:100px" value="<?php if ($registro_id!=0){ echo $periodo_ini; }  ?>" maxlength="60">
            </font>
          ...
          <td width="137" bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
            <input onChange="ActualizarNombre('<?php echo ($_SESSION[nomina]) ?>');" name="txtfechafinal" type="text" id="txtfechafinal" style="width:100px" value="<?php if ($registro_id!=0){ echo $periodo_fin; }  ?>" maxlength="60">
          ...
          <td width="151" bgcolor="#FFFFFF" class="ewTableAltRow"><font size="2" face="Arial, Helvetica, sans-serif">
            <input name="txtfechapago" onChange="ActualizarNombre2();" type="text" id="txtfechapago" style="width:100px" value="<?php if ($registro_id!=0){ echo $fecha_pago; }  ?>" maxlength="60">
