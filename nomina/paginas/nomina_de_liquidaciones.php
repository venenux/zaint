...
/////////////////////////////
$nombre_modulo=$termino.' de Liquidaciones';
...
$campo_9='';
$campo_10='';
$documento_list='nomina_de_liquidaciones.php';
$documento_edit='ag_nomina_liquidaciones.php';
...
{
//AbrirVentana('buscar_empleado2.php',550,650,0);
	if(bandera==0)
	{
		parent.cont.location.href="buscar_empleado2.php?codigo_nomina="+id+"&codt="+codtip;//+"&concepto="+id+"&ficha=
	}
	else if(bandera==1)
	{
		parent.cont.location.href="movimientos_nomina_liquidaciones.php?codigo_nomina="+id+"&codt="+codtip+"&bandera="+bandera	
	}
}
...
function enviar(op,id,nomina,codtip){
	...
	if (op==5){	// Movimiento de Nómina
		parent.cont.location.href="movimientos_nomina_liquidaciones.php?codigo_nomina="+id+"&codt="+codtip+"&prestaciones="+1
	...
	if (op==6){	//CERRAR NÓMINA
		if (confirm("Esta seguro que desea cerrar esta <?echo $termino?> ?"))
		{
			...
			cerrar_nomina.open("GET", "cerrar_nomina_liquidaciones.php?codigo_nomina="+id, true)
			cerrar_nomina.onreadystatechange=function() 
			{
				if (cerrar_nomina.readyState==4)
				{
					...
					parent.cont.location.href="nomina_de_liquidaciones.php"
				}
			}
...
{	
	...
	$strsql= "select COUNT(*) from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia='10'";
	$result =sql_ejecutar($strsql);	
	$fila = fetch_array($result);
	
	$num_total_registros = $fila[0];
	
	$strsql= "select * from $nombre_tabla where tipnom = ".$_SESSION['codigo_nomina']." and codtip = '".$_SESSION['codigo_nomina']."' and frecuencia='10' order by $campo_1,$campo_2 ";
	$result =sql_ejecutar($strsql);		
	$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
$conexion=conexion();
...
$consulta_max="select max(codnom) as maximo from nom_nominas_pago where tipnom='".$_SESSION['codigo_nomina']."'";
...
<tr>
<td width="100%"><strong> <font color="#000066"><a href="nomina_de_pago.php"><img src="images/Clipboard.png" width="23" height="21" border="0" align="absmiddle" /></a> Lista de <?echo $termino?>s de liquidacion.- <?echo $termino?>: <?php echo ($_SESSION[nomina]) ?> </font></strong></td>
<td width="2%" align="right">
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td><div align="right"><?php btn('add','ag_nomina_liquidaciones.php') ?></div></td>
<td><div align="right"> 
<?php btn('back','menu_transacciones.php') ?>
</div></td>
</tr>
</table></td>
</tr>
...
{ 
$consulta="select * from nom_movimientos_nomina where tipnom='".$_SESSION['codigo_nomina']."' and codnom='".$fila[$campo_1]."' ";
...
<td><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:BuscarPersonal(<?php echo($fila[$campo_1]); ?>,<? echo $fila['codtip']; ?>,<? echo $bandera; ?>);"><img src="images/view.gif" title="Movimientos" width="16" height="16" border="0" align="absmiddle" ></a></font></td>	
<?if($fila["status"]=="A"){?>
...
{
?>
<?if($fila_max['maximo']==$fila['codnom']){icono("javascript:enviar(7,$fila[$campo_1],0,0)","Abrir $termino","ok.gif");}?>
<td width="3%" >&nbsp;</td>
<td width="3%" >&nbsp;</td>
<td width="3%" >&nbsp;</td><?
}?>
