...
function Aceptar(variable)
{
	...
	opener.document.location.href="movimientos_nomina_pago.php?codigo_nomina="+document.frmPrincipal.codnomm.value+"&codt="+document.frmPrincipal.codtt.value+"&pag="+document.frmPrincipal.numpagg.value+"&ficha="+variable
	CerrarVentana();
}
...
{	
	...
	$strsql= "select * from $nombre_tabla where tipnom=$codigo_nomina and estado='Activo' order by $campo_4,$campo_3 LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);
}
?>
