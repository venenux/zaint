...
function enviar(op,id){
	...
	if (op==1){		// Opcion de Agregar
		...
		document.frmPrincipal.action="ag_subniveles.php";
	...
	if (op==3){		// Opcion de Eliminar
		...
		{					
			document.frmPrincipal.registro_id.value=id;
			document.frmPrincipal.op.value=op;
			//document.frmPrincipal.nivel.value=nivel;
  			document.frmPrincipal.submit();
		}		
	}
}
...
if (isset($HTTP_GET_VARS['nivel'])){
$nivel=$HTTP_GET_VARS['nivel'];}
else{
$nivel=$_POST['nivel'];}
$tabla_nivel = "nomnivel".$nivel;

// obtiene el nombre del nivel por como este configurado
// en la table empresas

$query="select nomniv".$nivel." from nomempresa";
...
if ($op==3) //Se presiono el boton de Eliminar
	...
	$query="delete from $tabla_nivel where codorg=$registro_id";	
...
elseif ($cadena <> "") 		// Condicion para filtrado
	{   
		...
		$strsql="select COUNT(*) from $tabla_nivel ";
		...
		$strsql="select * from $tabla_nivel";
		$strsql=filtrado($criterio,$cadena,$strsql,"codorg","descrip");		
...
else					// No se filtra y se muestran todos los datos
{
	...
	$strsql= "select * from $tabla_nivel order by codorg,descrip LIMIT $TAMANO_PAGINA OFFSET $limit";
	//$strsql= "select * from nomcategorias order by codorg,descrip LIMIT $TAMANO_PAGINA OFFSET $limit";
	$result =sql_ejecutar($strsql);		
    $total_paginas = ceil($num_total_registros / $TAMANO_PAGINA);	
}
?>
...
<form action="" method="post" name="frmPrincipal" id="frmPrincipal">
  <table width="100%" class="tb-tit">
    <tr>
      <td class="row-br"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="96%"><strong> <font color="#000066"> Subniveles de <?php echo $nombre_nivel; ?> </font></strong></td>
          <td width="2%"><?php btn('back','niveles_funcionales.php') ?></td>
          <td width="2%" align="right"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td><div align="right">
                <?php 
				btn('add','enviar(1,0);',2); ?>
              </div></td>
            </tr>
          </table></td>
        </tr>
  ...
  <table width="100%" border="0" class="ewTableHeader">
    <tr class="">
      ...
      <td width="17"><font size="2" face="Arial, Helvetica, sans-serif" class="ewTableHeader">
        <?php 
		btn('show_all','subniveles.php?nivel='.$nivel); 
		?>  		      
      </font></td>
  ...
  <table width="100%" border="0" bordercolor="#0066FF" bgcolor="#FFFFFF" class="ewTable" id="lst"  cellspacing="0" cellpadding="0">
    <tr bgcolor="#CCCCCC" class="ewTableHeader">
      <td width="20%" height="21" align="right" class="phpmakerlist"><div align="left" class="tb-head"> <font size="2" face="Arial, Helvetica, sans-serif"> Codigo </font></div></td>
      <td width="72%" class="phpmakerlist"><div align="left"> <font size="2" face="Arial, Helvetica, sans-serif"> Descripci&oacute;n </font></div></td>
    ...
    <tr>
      ...
      <td height="20" bgcolor="#DFDFDF" class="ewTableRow"><div align="left" class="row-even"><font size="2" face="Arial, Helvetica, sans-serif">
        <?php 
		  echo $fila[codorg]; 	// codigo del sub nivel
		  ?>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="left"><font size="2" face="Arial, Helvetica, sans-serif">
        <?php 
	  		echo $fila[descrip];  	// descripcion del sub nivel
	  	?>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(2); ?>,<?php echo($fila[codorg]); ?>);"><img src="img_sis/ico_edit.gif" alt="Modifica el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a>
                <label></label>
      </font></div></td>
      <td bgcolor="#DFDFDF" class="ewTableRow"><div align="center"><font size="2" face="Arial, Helvetica, sans-serif"><a href="javascript:enviar(<?php echo(3); ?>,<?php echo($fila[codorg]); ?>);"><img src="../imagenes/delete.gif" alt="Elimina el Registro Actual" width="16" height="16" border="0" align="absmiddle" ></a></font></div></td>
    </tr>
    <?php   
  	}//fin del ciclo while
	...
	?>
          ...
          <table border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <?php } else { ?>
              <td><a href="subniveles.php?nivel=<?php echo $nivel; ?>&pagina=1&criterio=<?php $criterio;?>"><img src="images/first.gif" alt="Primera" width="16" height="16" border="0"></a></td>
              <?php } ?>
              <!--previous page button-->
              <?php if ($PrevStart == $inicio) { ?>
              <td><img src="images/prevdisab.gif" alt="Anterior" width="16" height="16" border="0"></td>
              <?php } else { ?>
              <td><a href="subniveles.php?nivel=<?php echo $nivel; ?>&pagina=<?php echo $pagina-1; ?>&criterio=<?php $criterio;?>"><img src="images/prev.gif" alt="Anterior" width="16" height="16" border="0"></a></td>
              <?php } ?>
              <!--current page number-->
              ...
              <?php } else { ?>
              <td><a href="subniveles.php?nivel=<?php echo $nivel; ?>&pagina=<?php echo $pagina+1; ?>&criterio=<?php $criterio;?>"><img src="images/next.gif" alt="Siguiente" width="16" height="16" border="0"></a></td>
              <?php  } ?>
              <!--last page button-->
              <?php if ($NextStart == $inicio) { ?>
              <td><img src="images/lastdisab.gif" alt="Ultima" width="16" height="16" border="0"></td>
              <?php } else {?>
              <td><a href="subniveles.php?nivel=<?php echo $nivel; ?>&pagina=<?php  echo $LastStart; ?>&criterio=<?php $criterio;?>
