...
function Header()
{
	...
	$this->SetFont('Arial','B',12);
     	$this->Ln(30);
	$this->Cell(188,5,'DOTACION DE TEXTOS Y UTILES ESCOLARES',0,1,'C');
...
function detalle($edad1,$edad2,$sexo,$parentesco,$nivel,$nomina){
	...
	if($nivel!='Todos'){
		$cadenaselect=$cadenaselect." and niveledu='".$nivel."'";
	}
	if($nomina!='Todos'){
	...
	while($fila=fetch_array($query)){
		...
		if($codper!=$cod_persona){
			...
			if($anos>=$edad1 && $anos<=$edad2){
				...
				$cod_persona=$codper;
				$this->SetFont("Arial","I",10);
				$this->SetWidths(array(75,18,35,60));
				$this->SetAligns(array('C','C','C','C'));
				$this->Setceldas(array(1,1,1,1));
				$this->Setancho(array(5,5,5,5));
				$this->Row(array('Nombre y Apellido','Edad','Nivel Educativo',utf8_decode('Institución')));
			}
		}
		...
		if($anos>=$edad1 && $anos<=$edad2){
			...
			if(strlen($fila['institucion'])>=30 && (strlen($cade)>=43)){
			...
			$this->Row(array($fila['apellido'].', '.$fila['nombre'],$anos,$fila['niveledu'],$fila['institucion']));
			$cantidad_registros-=1;
		}
