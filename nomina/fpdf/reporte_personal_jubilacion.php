...
// 			$this->Cell(188,5,$cantidad_registros,0,1);
			if($tipo==2){
				$anti=antiguedad($fila['fechaing'],date("Y-m-d"),"A");
			...
			if($tipo==1){
				...
				$anos=antiguedad($fila['fecnac'],date("Y-m-d"),"A");
				$this->Row(array($fila['ficha'],utf8_decode($fila['apenom']),number_format($fila['cedula'],0,'.','.'),utf8_decode($fetchcargo['des_car']),date("d/m/Y",strtotime($fila['fecing'])),$anos,date("d/m/Y",strtotime($fila['fecharetiro'])),$fila['antiguedadap']));$cantidad_registros-=2;
				//instituciones 
				$this->SetWidths(array(90,40,90,40));
				$this->SetAligns(array('C','C','C','C'));
				$this->Setceldas(array(1,1,1,1));
