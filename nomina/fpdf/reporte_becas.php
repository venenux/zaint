...
function detalle($edad1,$edad2,$sexo,$parentesco,$nivel,$nomina){
	$this->SetFont("Arial","B",12);
	if($edad1!=$edad2){
	...
	if($parentesco!='Todos'){
		$cadenaselect=$cadenaselect.' and nf.codpar='.$parentesco;
	}
	if($nomina!='Todos'){
		$cadenaselect=$cadenaselect.' and np.tipnom='.$nomina;
	}
	if($nivel!='Todos'){
		$cadenaselect=$cadenaselect." and nf.niveledu='".$nivel."'";
	
	}
	
	
	$consulta="select * from nompersonal as np INNER JOIN nomfamiliares as nf ON np.ficha=nf.ficha and nf.cedula=np.cedula and np.estado='Activo' and nf.beca=1  $cadenaselect ORDER BY np.tipnom,np.codnivel4,np.apenom";
	...
	while($fila=fetch_array($query)){
		...
		if($codper!=$cod_persona){
			...
			if($anos>=$edad1 && $anos<=$edad2){
				...
				$this->Cell(188,8,'('.$fila['ficha'].')'.utf8_decode($fila['apenom']).utf8_decode(' - Cédula: ').number_format($fila['cedula'],0,'.','.').' -  Cargo: '.$rc['des_car'],0,1,'L');
				
				$cantidad_registros-=2;
				$cod_persona=$codper;
				$this->SetFont("Arial","I",10);
				$this->SetWidths(array(70,30,18,60,12));
				$this->SetAligns(array('C','C','C','C','C'));
				$this->Setceldas(array(1,1,1,1,1));
				$this->Setancho(array(5,5,5,5,5));
				$this->Row(array('Nombre y Apellido','Nivel Educativo','F.Notas',utf8_decode('Institución'),'Prom.'));
			}
		}
		...
		if($anos>=$edad1 && $anos<=$edad2){
			...
			if(strlen($fila['institucion'])>=30 && (strlen($cade)>=40)){
			...
			$this->Row(array($fila['apellido'].', '.$fila['nombre'],$fila['niveledu'],date("d/m/Y",strtotime($fila['fecha_beca'])),$fila['institucion'],number_format($fila['promedionota'],2,',','.')));
			$totalbecado+=1;
			$cantidad_registros-=1;
		}
	...
	$this->Ln();
	$this->SetFont('Arial','B',12);
	$this->Cell(188,5,'Total de Becados => '.$totalbecado,0,1,'R');
...
$nivel=$_POST['niveledu2'];
$nomina=$_POST['nomina'];

$pdf->detalle($edad1,$edad2,$sexo,$parentesco,$nivel,$nomina);
