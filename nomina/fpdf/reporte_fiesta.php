...
function Header()
{
	...
	$this->SetFont('Arial','B',12);
     	$this->Ln(30);
	$this->Cell(188,5,utf8_decode('FIESTA INFANTIL NAVIDEÑA'),0,1,'C');
...
function detalle($edad1,$edad2,$sexo,$parentesco,$nivel,$nomina){
	...
	while($fila=fetch_array($query)){
		...
		if($codper!=$cod_persona){
			...
			if($anos>=$edad1 && $anos<=$edad2){
				...
				$cod_persona=$codper;
				$this->SetFont("Arial","I",10);
				$this->SetWidths(array(75,18,60,35));
				$this->SetAligns(array('C','C','C','C'));
				$this->Setceldas(array(1,1,1,1));
				$this->Setancho(array(5,5,5,5));
				$this->Row(array('Nombre y Apellido','Edad','Fecha de Nacimiento',utf8_decode('Sexo')));
			}
		}
		...
		if($anos>=$edad1 && $anos<=$edad2){
			...
			$this->Row(array($fila['apellido'].', '.$fila['nombre'],$anos,date("d/m/Y",strtotime($fila['fecha_nac'])),$fila['sexo']));
			$cantidad_registros-=1;
	...
	$this->Ln();
	$this->SetFont("Arial","B",12);
	$this->Cell(188,5,'Total de Trabajadores => '.$TOTALTRABAJADORES,0,1,'R');
