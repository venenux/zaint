...
function header(){
	...
	$this->Cell(150,5,'LEY DE POLITICA HABITACIONAL',0,1,'L');
...
function Cuentas($codcon,$codcon_pat){
	
	$conexion=conexion();
	$consulta="select * from nompersonal where tipnom='".$_SESSION['codigo_nomina']."' order by codnivel4, apenom";
	$resultado=query($consulta,$conexion);
	cerrar_conexion($conexion);
	// llamado para hacer multilinea sin que haga salto de linea
	
	$this->SetFont('Arial','',12);
        $this->SetWidths(array(108,30,25,25));
	$this->SetAligns(array('C','C','C','C'));
        $this->Setceldas(array('TB','TB','TB','TB'));
	$this->Setancho(array(5,5,5,5));
        $this->Row(array('APELLIDOS Y NOMBRES',utf8_decode('CÉDULA'),'INGRESO','APORTE'));
	...
	while($totalwhile>=$contar)
	{
		...
		if($fila_mov['monto']!=0)
		{
			...
			$this->Row(array(utf8_decode($fila['apenom']),utf8_decode($cedula),fecha($fila['fecing']),number_format($fila_mov['monto']+$fila_mov_pat['monto'],2,',','.')));
