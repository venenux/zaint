...
function detalle($gerencia,$nomina,$desde,$hasta,$cargo,$entrega){
	...
	if($entrega=='Si'){
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula  and trim(ne.tipo_registro)='Entrega de Uniformes' and  np.estado='Activo' and fecha_salida between '$desdea' and '$hastaa'  $cadenaselect  group by np.codcargo ORDER BY np.codnivel4,np.apenom";
	}
	...
	while($fila=fetch_array($query)){
		...
		$code=$fila['codcargo'];
		$conexion=conexion();
		$conadic="select * from nomcampos_adic_personal where ficha='$fila[ficha]'  and (id=19 or id=20 or id=47 or id=21 or id=48 or id=49)  ";
