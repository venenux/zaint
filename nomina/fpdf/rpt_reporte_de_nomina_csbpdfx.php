...
function header(){
	...
	$fila2 = fetch_array($result2);
        ...
        $this->SetFont("Arial","",10);
     	...
     	$this->Cell(88,6,$_SESSION['termino'].': '. $_GET['nomina_id'] .' - '. $_SESSION[nomina],0,0,'C');
     	$this->Ln(10);
	 $this->SetFont("Arial","",10);
...
function personas($nomina_id,$codt,$pdf){
	...
	$conexion=conexion();
	$query="select per.*,car.* from nompersonal as per LEFT join nom_movimientos_nomina as nom on (per.ficha = nom.ficha) left join nomcargos as car on per.codcargo = car.cod_car where  nom.codnom = '$nomina_id' and nom.tipnom = ".$codt." and per.tipnom=".$codt." group by per.ficha order by per.codnivel4, per.ficha";
	...
	while ($fila = fetch_array($result))
	{
		...
		if($gertmp!=$fila['codnivel4'])
		{
			...
			while ($row = fetch_array($result1))
			{
				...
				$this->Row(array($row[codcon].'  '.$row[descrip],$row[tipcon],$valor1,$valor2));
	...
	$pdf->firmas($total_asig,$total_dedu);
...
function firmas($total_asig,$total_dedu){
