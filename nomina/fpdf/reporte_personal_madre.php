...
function detalle($gerencia,$nomina){
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal where codnivel4<>'' $cadenaselect and sexo='Femenino' ORDER BY  codnivel4,tipnom,apenom ";
	}else{
		
		$consulta="select * from nompersonal where codnivel4<>'' $cadenaselect and sexo='Femenino' ORDER BY tipnom, codnivel4,apenom ";
	...
	while($fila=fetch_array($query)){
		$consulta_famili=query("select * FROM nomfamiliares where cedula=".$fila['cedula']." and codpar=3",$conexion);
		if(num_rows($consulta_famili)!=0){
