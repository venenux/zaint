		...
		if($gerencia!=$fila['ger4']){
			...
			if ($gerencia!=""){
				...
				if(num_rows($query_con)+5<=$cantidad_registros){
					...
					$cantidad_registros=42;
					$this->SetFont('Arial','I',8);
					$consulta_n4="SELECT descrip FROM nomnivel4 WHERE codorg='".$gerencia."'";
...
// 			}
			...
			$cons="select valor from nomcampos_adic_personal where ficha = '".$sficha."' and tiponom='".$codt."' and id=7";
			$ress=query($cons,$conexion);
			$fetch33=fetch_array($ress);
			$result2=query($query,$conexion);			
...
// 			$this->Cell(188,5,$cantidad_registros,0,1);
			if(num_rows($result1)+5<=$cantidad_registros){
				...
				$this->Row(array(utf8_decode('Cédula: '.number_format($fila['cedula'],0,',','.'))."     Sueldo: ".$fila['suesal']."      Comp.: ".$fetch33['valor']."      Factor: 0.11",'Ficha: '.$fila['ficha']."          F. ing.: ".fecha($fila[fecing])."   Dias a pagar: ".number_format((($row2[valor]*60)/365),2,',','.')));
				$this->Ln(2);
				$this->SetFont('Arial','',8);
				$this->Cell(90,5,'Concepto',1,0,'C');
				$this->Cell(24,5,utf8_decode('Días.'),1,0,'C');
...
// 			$this->Cell(188,5,$cantidad_registros,0,1);
			...
			while ($row = fetch_array($result1))
			{
				...
				if($row[valor]==0)
					$valor333='';
				$this->Row(array($row[codcon].'  '.utf8_decode($row[descrip]),$valor333,$valor1,$valor2));
				$cantidad_registros-=1;
...
function firmas($total_asig,$total_dedu,$cantidad_registros){
	...
	$consultaa="SELECT ficha, apenom FROM nompersonal WHERE tipnom =".$_SESSION['codigo_nomina']." and estado<>'Egresado'". " and estado<>'Egresado de nomina de pago'";
