...
function detalle($desde,$hasta,$cargo,$entrega,$gerencia,$nomina){

	
	$desdea=fecha_sql($desde);
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nomexpediente as ne  INNER JOIN nompersonal as np ON  np.estado='Activo' and np.cedula=ne.cedula and ne.tipo_registro='Entrega de Uniformes' $cadenaselect and ne.fecha_salida between '$desdea' and '$hastaa' ORDER BY np.codnivel4,np.tipnom,np.apenom ";


	}else{
		$consulta="select * from nomexpediente as ne INNER JOIN nompersonal as np ON  np.estado='Activo' and ne.cedula=np.cedula and ne.tipo_registro='Entrega de Uniformes' $cadenaselect and ne.fecha_salida between '$desdea' and '$hastaa'  ORDER BY np.tipnom, np.codnivel4,np.apenom ";
	...
	if($entrega='No'){
		$consulta="select * from nompersonal where cedula not in ( select ne.cedula from nomexpediente as ne INNER JOIN nompersonal as np ON  np.estado='Activo' and ne.cedula=np.cedula and ne.tipo_registro='Entrega de Uniformes' $cadenaselect and ne.fecha_salida between '$desdea' and '$hastaa')";
	...
	while($fila=fetch_array($query)){
		...
		if($code!=$cod_especialidad){
			...
			if($fila['tmono']!=''){
				...
				$this->Row(array($fila['ficha'],$fila['apenom'],$fila['tcamisa'],$fila['tpantalon'],$fila['tzapato'],$fila['tchaqueta'],$fila['tbata'],$fila['tmono']));
				$cantidad_registros-=1;
...
$nomina=$_POST['nomina'];
	$pdf->detalle($desde,$hasta,$cargo,$entrega,$gerencia,$nomina);
