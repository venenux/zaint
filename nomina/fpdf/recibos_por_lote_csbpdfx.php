...
function personas($nomina_id,$codtp){
	...
	while ($fila=fetch_array($result_lote))
	{
		...
		$this->Setceldas(array(0,0));
		$this->Setancho(array(5,5));
		$this->Row(array('Cargo: '.$row[des_car],'Banco/Cuenta: '.$row[des_ban] . ' BOD - ' . $fila[cuentacob]));
		$this->Ln(1);
		...
		$this->Cell(1,0,utf8_decode('--------------------------------------------------------------------'),0,0,'L');
              ...
              $result =query($query,$conexion);
             	$this->Cell(80,7,utf8_decode('Código y Descripción de Concepto'),0,0,'R');
		$this->Cell(120,7,utf8_decode('Asignación'),0,0,'C');
		$this->Cell(-15,7,utf8_decode('Deducción'),0,1,'R');
		$this->Ln(2);
		...
		while ($row = mysql_fetch_array($result))
		{
			...
			$this->SetFont('Sanserif','',9);
			$this->SetWidths(array(120,34,34));
			$this->SetAligns(array('L','R','R'));
			$this->Setceldas(array(0,0,0));
			$this->Setancho(array(5,5,5));
			$this->Row(array($row[codcon] . ' - ' . utf8_decode($row[descrip]),$asig,$dedu));
			$asig='';
			$dedu='';
