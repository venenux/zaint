...
function detalle($gerencia,$nomina,$desdea1,$hastaa1){

	$desdea=date("Y-m-d",strtotime($desdea1));
	$hastaa=date("Y-m-d",strtotime($hastaa1));

	$this->Cell(270,5,'LISTADO DE TRABAJADORES CON ANTICIPOS DESDE '.$desdea1.' HASTA '.$hastaa1,0,0,'C');
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula and ne.tipo_registro='Antic. prestaciones' and fecha_salida between '$desdea' and '$hastaa'  $cadenaselect  ORDER BY  np.codnivel4,np.tipnom,np.apenom";

	}else{
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula and ne.tipo_registro='Antic. prestaciones' and fecha_salida between '$desdea' and '$hastaa'  $cadenaselect  ORDER BY np.tipnom, np.codnivel4,np.apenom";
...
// 			$this->Ln();
			...
			if($cod_trabajador!=$fila['ficha']){
				...
				//permisos
				$this->SetWidths(array(200,30,30));
				...
				$cantidad_registros-=1;
				$this->SetFont("Arial","I",11);
				$this->Row(array('Motivo','Fecha','Monto'));
				$this->SetFont("Arial","I",8);
				...
				$this->Row(array($fila['descripcion'],date("d/m/Y",strtotime($fila['fecha_salida'])),$fila['monto']));
...
$nomina=$_POST['nomina'];
$desdea=$_POST['fechadesdea'];
$hastaa=$_POST['fechahastaa'];
$pdf->detalle($gerencia,$nomina,$desdea,$hastaa);
