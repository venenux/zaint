...
function detalle($gerencia,$nomina,$tipo){

	

	$this->Cell(270,5,'LISTADO DE TRABAJADORES CON MOVIMIENTOS DE PERSONAL',0,0,'C');
	...
	if($gerencia!='Todos' && $nomina=='Todos'){
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula and ne.tipo_registro='Movimiento de Personal'   $cadenaselect  ORDER BY  np.codnivel4,np.tipnom,np.apenom,ne.fecha_salida";

	}else{
		$consulta="select * from nompersonal as np INNER JOIN nomexpediente as ne ON np.cedula=ne.cedula and ne.tipo_registro='Movimiento de Personal'  $cadenaselect  ORDER BY np.tipnom, np.codnivel4,np.apenom,ne.fecha_salida";
...
// 			$this->Ln();
			...
			if($cod_trabajador!=$fila['ficha']){
				...
				if($tipo=='Todos'){
					$this->Row(array('Tipo',utf8_decode('Fecha posecion'),utf8_decode('Fecha Culm.'),utf8_decode('Anterior'),utf8_decode('Nuevo')));
				}else{
					if($tipo=='Traslado de cargo' || $tipo=='Comision de servicios'){
						$this->Row(array('Tipo',utf8_decode('Fecha posecion'),utf8_decode('Fecha Culm.'),utf8_decode('Cargo'),utf8_decode('Cargo Nuevo')));
					}
					if($tipo=='Traslado de nomina'){
						$this->Row(array('Tipo',utf8_decode('Fecha posecion'),utf8_decode('Fecha Culm.'),utf8_decode('Nomina'),utf8_decode('Nomina Nueva')));
					}
					if($tipo=='Traslado de gerencias'){
						$this->Row(array('Tipo',utf8_decode('Fecha posecion'),utf8_decode('Fecha Culm.'),utf8_decode('Gerencia'),utf8_decode('Gerencia Nueva')));
					}
				}
				$this->SetFont("Arial","I",8);
				...
				$tipo_compa=$fila['tipo_tiporegistro'];
				if($tipo_compa=='Traslado de cargo' || $tipo_compa=='Comision de servicios'){
					//cargo actual
					$consultacargo1="select * from nomcargos where cod_car='$fila[cod_cargo]'";
					$querycargo1=query($consultacargo1,$conexion);
					$fetchcargo1=fetch_array($querycargo1);
					//cargo nuevo
					$consultacargo2="select * from nomcargos where cod_car='$fila[cod_cargo_nuevo]'";
	...
	// 				$this->Cell(188,5,strlen($fetchcargo1['des_car']),0,1);
					...
					if($fila['fecha_retorno']=='0000-00-00')
						...
						$fechar=date("d/m/Y",strtotime($fila['fecha_retorno']));
					if($fila['fecha_salida']=='0000-00-00')
						$fechas='-';
					else
						$fechas=date("d/m/Y",strtotime($fila['fecha_salida']));
					$this->Row(array($fila['tipo_tiporegistro'],$fechas,$fechar,utf8_decode($fetchcargo1['des_car']),utf8_decode($fetchcargo2['des_car'])));
				}
				if($tipo_compa=='Traslado de nomina'){
					//cargo actual
					$consultacargo1="select * from nomtipos_nomina where codtip='$fila[nomina_anterior]'";
					$querycargo1=query($consultacargo1,$conexion);
					$fetchcargo1=fetch_array($querycargo1);
					//cargo nuevo
					$consultacargo2="select * from nomtipos_nomina where codtip='$fila[nomina_nueva]'";
					$querycargo2=query($consultacargo2,$conexion);
					$fetchcargo2=fetch_array($querycargo2);
					if(strlen($fetchcargo1['descrip'])>45){
						$var1=1;
					}else{
						$var1=0;
					}
	// 				$this->Cell(188,5,strlen($fetchcargo2['des_car']),0,1);
	// 				$this->Cell(188,5,strlen($fetchcargo1['des_car']),0,1);
					if(strlen($fetchcargo2['descrip'])>45 && $var1==0){
						$this->Setancho(array(10,10,10,10,5));
					}
					if(strlen($fetchcargo2['descrip'])>45 && $var1==1){
						$this->Setancho(array(10,10,10,5,5));
					}
					if(strlen($fetchcargo2['descrip'])<45 && $var1==0){
						$this->Setancho(array(5,5,5,5,5));
					...
					$this->Row(array($fila['tipo_tiporegistro'],date("d/m/Y",strtotime($fila['fecha_salida'])),date("d/m/Y",strtotime($fila['fecha_retorno'])),utf8_decode($fetchcargo1['descrip']),utf8_decode($fetchcargo2['descrip'])));
				}
				if($tipo_compa=='Traslado de gerencias'){
					//cargo actual
					$consultacargo1="select * from nomnivel4 where codorg='$fila[gerencia_anterior]'";
					$querycargo1=query($consultacargo1,$conexion);
					$fetchcargo1=fetch_array($querycargo1);
					//cargo nuevo
					$consultacargo2="select * from nomnivel4 where codtorg='$fila[gerencia_nueva]'";
					$querycargo2=query($consultacargo2,$conexion);
					$fetchcargo2=fetch_array($querycargo2);
					if(strlen($fetchcargo1['descrip'])>45){
						$var1=1;
...
$nomina=$_POST['nomina'];
$tipo=$_POST['tipo3'];
$pdf->detalle($gerencia,$nomina,$tipo);
