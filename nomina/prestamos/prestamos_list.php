...
function confirmar2(valor,cedula)
{
	if (confirm("seguro desea eliminar este registro") == true) 
		window.location.href="prestamos_list.php?cod_eliminar="+valor+"&cedula="+cedula
}
...
{
	$consulta="SELECT * FROM nomprestamos_detalles WHERE numpre=$_GET[cod_eliminar] AND estadopre='Cancelada'";
	...
	{
		$consulta="DELETE FROM nomprestamos_cabecera WHERE numpre=$_GET[cod_eliminar]";
		$resultado=query($consulta,$conexion);
		
		$consulta="DELETE FROM nomprestamos_detalles WHERE numpre=$_GET[cod_eliminar]";
		$resultado=query($consulta,$conexion);
	}
}
...
<?
titulo($modulo,"prestamos_agregar.php?cedula=$cedula","menu_prestamos.php","21");
?>
...
if($num_paginas!=0)
{
	...
	while($fila=fetch_array($resultado))
	{
		...
		icono("prestamos_edit.php?numpre=".$codigo, "Editar", "edit.gif");
