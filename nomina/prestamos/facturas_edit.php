...
{
	echo $consulta="update nomfacturas_detalles set estadopre='Cancelada', ee='1' WHERE numpre='$_GET[cod_editar]' AND numcuo='$_GET[cuota]'";
	if($resultadoCon=query($consulta,$conexion))	
	{
		
		echo "<script type='text/javascript'>";
		//alert("FACTURA TIENE CUOTAS CANCELADAS NO PUEDE ELIMINAR!!!")
		echo "parent.cont.location.href='facturas_edit.php?numpre=$_GET[cod_editar]'";
 		echo "</script>";
	...
	else
	{
		...
		//parent.cont.location.href="facturas_edit.php?numpre=$_GET[cod_editar]"
 		</script>
		<?php	
	}
}
...
// {
// 	$consulta="INSERT INTO nomfacturas_cabecera SET numpre=$_POST[numpre], ficha=$_POST[ficha], fechaapro='".fecha_sql($_POST[fechaap])."', fecpricup='".fecha_sql($_POST[fecha1])."', monto=$_POST[montopre], estadopre='Pendiente', detalle='$_POST[descrip]', codigopr='$_POST[tipo]', codnom=$_SESSION[codigo_nomina], totpres=$_POST[montopre]";
...
// 		?>
 		<script type="text/javascript">
// 		alert("FACTURAS GUARDADO EXITOSAMENTE!!!")
...
function confirmar2(numpre,cuota)
{
	if (confirm("Seguro desea cancelar esta cuota") == true) 
		window.location.href="facturas_edit.php?cod_editar="+numpre+"&cuota="+cuota
}
...
<tr>
<td width="100%"><strong> <font color="#000066"> Agregar factura </font></strong></td>
...
$consulta="SELECT * FROM nomfacturas_cabecera where numpre=$_GET[numpre]";
$resultado=query($consulta,$conexion);
$fetch=fetch_array($resultado);

$consulta="SELECT * FROM nomfacturas_detalles where numpre=$_GET[numpre]";
$resultado2=query($consulta,$conexion);
?>

<table width="100%" border="0"  id="lst"  cellspacing="0" cellpadding="0">
<tr class="tb-head"  style="font-weight : bold;"><td width="30%">Numero de factura:   <? echo $fetch[numpre];?><input type="hidden" name="numpre" id="numpre" value="<? echo $numpre;?>"></td>
...
<TR>
<TR>
<strong>Monto del factura:</strong> <input type="text" name="montopre" align="right" id="montopre" value="<? echo $fetch[monto]?>" maxlength="8" size="10"/><font color="Red">&nbsp;*</font>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<tr>
<td>
<strong>Descripción del factura:</strong> <input type="text" name="descrip" align="right" id="descrip" value="<? echo $fetch[detalle]?>" maxlength="300" size="50"/>
</td>
</TR>
<tr><TD>&nbsp;</TD></tr>
<tr>
