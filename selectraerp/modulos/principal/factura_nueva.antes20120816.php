...
if (isset($_POST["PFactura2"])) {
    ...
    # Se cambia el status del documento mercantil repectivo que se facturara y se le asocia la factura
    $tienePedido = $factura->ObtenerFilasBySqlSelect("SELECT * FROM pedido WHERE id_cliente=" . $_GET["cod"] . " AND cod_estatus = 1 AND id_factura = 0 AND id_pedido = " . $_POST['hijo']);
    $tieneNotaEntrega = $factura->ObtenerFilasBySqlSelect("SELECT * FROM nota_entrega WHERE id_cliente=" . $_GET["cod"] . " AND cod_estatus = 1 AND id_factura = 0 AND id_nota_entrega = " . $_POST['hijo']);
    $tieneCotizacion = $factura->ObtenerFilasBySqlSelect("SELECT * FROM cotizacion_presupuesto WHERE id_cliente=" . $_GET["cod"] . " AND cod_estatus = 1 AND id_factura = 0 AND id_cotizacion = " . $_POST['hijo']);
    if ($tienePedido) {
        $factura->ExecuteTrans("UPDATE pedido SET cod_estatus = 2, id_factura = " . $id_facturaTrans . " WHERE id_cliente = " . $_GET['cod'] . " AND id_pedido = " . $_POST['hijo']);
    }
    if ($tieneNotaEntrega) {
        $factura->ExecuteTrans("UPDATE nota_entrega SET cod_estatus = 2, id_factura = " . $id_facturaTrans . " WHERE id_cliente = " . $_GET['cod'] . " AND id_nota_entrega = " . $_POST['hijo']);
    }
    if ($tieneCotizacion) {
        $factura->ExecuteTrans("UPDATE cotizacion_presupuesto SET cod_estatus = 2, id_factura = " . $id_facturaTrans . " WHERE id_cliente = " . $_GET['cod'] . " AND id_cotizacion = " . $_POST['hijo']);
    }
    ########################################################################
    ...
    $detalles.="DIRECCION1: " . $obj_txt->formatearDatosCabecera($datos_cliente[0]['direccion']) . "\n";
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {
        $item_descripcion = $_POST["_item_descripcion"][$i];
        $detalle_item_instruccion = "
            ...
            VALUES (
                ...
                . $item_descripcion . "', '" . $_POST["_item_cantidad"][$i] . "', '" . $_POST["_item_preciosiniva"][$i] . "','"
        ...
        $descrip_producto = strlen($item_descripcion) < 39 ? $obj_txt->formatearDatosCabecera($item_descripcion/* $_POST["_item_descripcion"][$i] */, ".", 39) : substr($item_descripcion, 0, 39);
        ...
        #$cantidad = str_replace(".", ",", $_POST["_item_cantidad"][$i]);
        $cantidad = $obj_txt->formatearCantidadDecimales($_POST["_item_cantidad"][$i]);
        #$precio = str_replace(".", ",", $_POST["_item_preciosiniva"][$i]);
        $precio = $obj_txt->formatearCantidadDecimales($_POST["_item_preciosiniva"][$i]);
        #$iva = str_replace(".", ",", $_POST["_item_piva"][$i]);
        $iva = $obj_txt->formatearCantidadDecimales($_POST["_item_piva"][$i]);

        $whitespaces = 30 - (strlen($codigo_item) + strlen($cantidad));
        for ($j = 0; $j < $whitespaces; $j++) {
            $codigo_item .= " ";
        }
        #$codigo=$obj_txt->formatearLineasDetallesPago($codigo," ",39);
        #$linea_producto.=$descrip_producto . " " . $codigo . " " . str_replace('.', ',', $cantidad) . " " . str_replace('.', ',', $precio) . " " . str_replace('.', ',', $_POST["_item_piva"][$i]) . "\n";
    ...
    fwrite($archivo_spooler, $linea_producto);
    #$descuento = str_replace(".", ",", $_POST["input_totalizar_pdescuento_global"] > 0 ? $_POST["input_totalizar_pdescuento_global"] : 0);
    ...
    #$neto=str_replace(".", ",", $_POST["totalizar_base_imponible"]);
    ...
    #$total=str_replace(".", ",", $_POST["input_totalizar_total_general"]);
    $linea_producto.=$obj_txt->formatearLineasDetallesPago("TOTAL CANCELADO:", $obj_txt->formatearCantidadDecimales($_POST["totalizar_monto_cancelar"]/* $_POST["input_totalizar_total_general"] */)) . "\n";
    #$efectivo=str_replace(".", ",", $_POST["input_totalizar_monto_efectivo"]);
    ...
    #$cheque=formatearCantidadDecimales($_POST["input_totalizar_monto_cheque"]);
    ...
    #$linea_producto.="Tarjeta                  " . str_replace('.', ',', $_POST["input_totalizar_monto_tarjeta"]) . "\n";
    #$tarjeta=formatearCantidadDecimales($_POST["input_totalizar_monto_tarjeta"]);
    ...
    #$linea_producto.="COMENTARIO2:      LICORERIA LAS FLORES S.R.L            \n";
