...
if (isset($_POST["PFactura2"])) {
    ...
    VALUES(
        ...
        '" . $login->getUsuario() . "', '{$cod_estatus}', '{$_POST["forma_pago"]}'
        );";
    $factura->ExecuteTrans($sql_cabecera);
    $id_facturaTrans = $factura->getInsertID();
    /*
     * Codigo fuente aÃ±adido para la facturacion de pedidos, notas de entrega y
     * cotizaciones. Se cambia el status del documento mercantil respectivo que
     * se facturarÃ¡ y se le asocia la factura
     */
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {
        ...
        #$codigo_item = str_pad($codigo_item, $espacios, " ");
        ...
        if (!$tienePedido && !$tieneNotaEntrega) {
            ...
            if ($_POST["estado_entrega"] == 'Entregado') {
                ...
                # Verificar que se estÃ¡ facturando para no descontar de la existencia,
                # ya que el propio pedido descontÃ³
                #if (!$tienePedido) {
    ...
    $parametros_impresora_fiscal = $parametros->ObtenerFilasBySqlSelect("SELECT tipo_facturacion, impresora_serial FROM parametros_generales;");
    // Si el tipo de facturaciÃ³n es Fiscal
    if ($parametros_impresora_fiscal[0]['tipo_facturacion'] == 1) {
        ...
        $linea_producto.="SERIALIMP:        {$parametros_impresora_fiscal[0]['impresora_serial']}\n";
        $linea_producto.="COO-BEMATECH:     000123\n";

        fwrite($archivo_spooler, $linea_producto);
        fclose($archivo_spooler);
        /*
         * En este punto comienza el parseo de la BD (DBF) del SÃºper Spooler Fiscal
    ...
    if ($factura->errorTransaccion == 1) {
        ...
        /* $factura_pdf = "rpt_factura";
          if ($parametros_impresora_fiscal[0]['tipo_facturacion'] == 1) {
          $factura_pdf.=".php";
          } else if ($parametros_impresora_fiscal[0]['tipo_facturacion'] == 2) {
          $factura_pdf.="_formato_libre.php";
          } */
    ...
    ##########################################################################################
    # Codigo aÃ±adido para obtener los pedidos, notas de entrega y cotizaciones asociados a
