...
{/literal}
<script src="../../libs/js/entrada_almacen.js" type="text/javascript"></script>
...
{else}
    ...
    <tr style=" cursor: pointer;" class="detalle" bgcolor="#ececec">
        <td width="25" align="center">
        <img class="boton_detalle" src="../../libs/imagenes/drop-add.gif">
        ...
        <td>{$campos.id_transaccion}</td>
        <td>{$campos.descripcion}</td>
        <td>{$campos.fecha|date_format:"%d-%m-%Y"} </td>
        <td>{$campos.autorizado_por}</td>
        <td>{$campos.observacion}</td>
        <td>
        {if $campos.estado eq "Entregado"}
        <img title="Entregado" src="../../libs/imagenes/ico_ok.gif">
        {/if}

        {if $campos.estado eq "Pendiente"}
            <img title="Pendiente" src="../../libs/imagenes/ico_note_1.gif">
        {/if}
        <img style="cursor: pointer;" class="impresion" onclick="javascript: window.open('../../reportes/entrada_almacen.php?id_transaccion={$campos.id_transaccion}','')" title="Imprimir Detalle de Movimiento"  src="../../libs/imagenes/ico_print.gif">
        </td>
    </tr>
    {/foreach}
{/if}
