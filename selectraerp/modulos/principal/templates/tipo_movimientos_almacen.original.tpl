...
{else}
    ...
    {else}
        <tr  bgcolor="#e1e1e1">
    {/if}
        <td>{$campos.id_tipo_movimiento_almacen}</td>
        <td>{$campos.descripcion}</td>
        <td>{if $campos.operacion eq "+" }Entrada{/if}{if $campos.operacion eq "-" }Salida{/if}</td>
        <td>
            <img  style="cursor: pointer;" class="editar"  onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=edit&cod={$campos.id_tipo_movimiento_almacen}'" title="Editar" src="../../libs/imagenes/edit.gif">
            <img  style="cursor: pointer;" class="eliminar" onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=delete&cod={$campos.id_tipo_movimiento_almacen}'" title="Eliminar"  src="../../libs/imagenes/delete.gif">
