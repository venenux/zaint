        ...
        {else}
            {foreach from = $registros key = i item = campos}
                {if $i%2==0}
                    ...
                    <tr bgcolor="#e1e1e1">
                    {/if}
                    <td>{$campos.id_factura}</td>
                    <td>{$campos.cod_factura}</td>
                    <td>{$campos.nombre}</td>
                    <td>{$campos.rif}</td>
                    <td>{$campos.fechaFactura}</td>
                    <td>{$campos.totalizar_total_general}</td>
                    <td style="cursor: pointer; text-align:center;"><img class="impresion" onclick="javascript: window.open('../../reportes/rpt_factura.php?codigo={$campos.cod_factura}','')" title="Imprimir Factura"  src="../../libs/imagenes/ico_print.gif"/></td>
                    <td style="cursor: pointer; text-align:center;"><img class="anular" onclick="javascript: window.location.href = '?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=devolver_ps&codigo={$campos.cod_factura}'" title="Anular Factura"  src="../../libs/imagenes/ico_basket.gif"/></td>
                </tr>
