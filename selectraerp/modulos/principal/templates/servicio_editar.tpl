...
<tr>
    ...
    <td>
        <select name="cod_departamento" id="cod_departamento">
          {html_options values=$option_output_departamentos output=$option_values_departamentos selected=$option_select_departamentos}
        </select>
    </td>
</tr>
<tr>
    ...
    <td>
        <select name="cod_grupo" id="cod_grupo">
          {html_options values=$option_output_grupo output=$option_values_grupo selected=$option_select_grupo} 
        </select>
    </td>
</tr>
<tr>
...
<select name="cod_linea" id="cod_linea">
    {html_options values=$option_output_linea output=$option_values_linea selected=$option_select_linea}
</select>
    </td>
</tr>
...
{foreach from=$islr key=i item=campos}
	{assign var = xxx value = 0}
	<tr >
	<TD colspan="3" width="150px" class="tb-head" align="left">
	<STRONG>{$campos.descripcion}</STRONG>
	</TD>
	<td>
	{foreach from=$islrItem item=camposs}
		{if $camposs eq $campos.cod_impuesto}
			<input class="campo_decimal" type="checkbox"  checked="true" value="{$campos.cod_impuesto}" name="{$campos.cod_impuesto}">
...
<table width="100%" border="0">
    ...
    <td>
<input type="hidden" name="id_item" id="id_item" value="{$campos_item[0].id_item}">
