...
{else}
    ...
    {else}
        <tr  bgcolor="#e1e1e1">
    {/if}
        <td>{$campos.cod_impuesto_ica}</td>
        <td>{$campos.actividad}</td>
        <td>{$campos.agrupacion}</td>
        <td>{$campos.cod_actividad_ciu}</td>
        <td>{$campos.descripcion}</td>
        <td>{$campos.tarifa}</td>
        <td>
            <img  style="cursor: pointer;" class="editar"  onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=edit&cod={$campos.cod_impuesto_ica}'" title="Editar" src="../../libs/imagenes/edit.gif">
            <img  style="cursor: pointer;" class="eliminar" onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=delete&cod={$campos.cod_impuesto_ica}'" title="Eliminar"  src="../../libs/imagenes/delete.gif">
        </td>
    </tr>
    {assign var = ultimo_cod_valor value=$campos.cod_impuesto_ica}
    {/foreach}
{/if}
</tbody>
</table>
