        ...
        {else}
            {foreach from = $registros key = i item = campos}
                {if $i%2==0}
                    ...
                    <tr  bgcolor="#e1e1e1">
                    {/if}
                    <td >{$campos.cod_proveedor}</td>
                    <td >{$campos.rif}</td>
                    <td >{$campos.descripcion}</td>
                    <td>{$campos.clase_proveedor}</td>
                    <td>
                        <img  style="cursor: pointer;" class="editar"  onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=edit&cod={$campos.id_proveedor}'" title="Editar Información del Proveedor" src="../../libs/imagenes/edit.gif">
                        <img  style="cursor: pointer;" class="editar"  onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=newCompra&cod={$campos.id_proveedor}'" width="17" height="17" title="Generar Nueva Compra" src="../../libs/imagenes/40.png">
                    </td>
                </tr>
