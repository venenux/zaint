...
{/literal}
<form action="?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}" method="post">
    <table width="100%">
        <tr class="row-br">
            <td>
        ...
        {else}
            {foreach from = $registros key = i item = campos}
                {if $i%2==0}
                    ...
                    <td>{$campos.total_inventario}</td>
                    <td>{$campos.existencia_min}</td>
                    <td>{$campos.existencia_max}</td>
                    <td>
                        <img  style="cursor: pointer;" class="editar"  onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=edit&cod={$campos.id_item}'" title="Editar" src="../../libs/imagenes/edit.gif">
                        <img  style="cursor: pointer;" class="eliminar" onclick="javascript: window.location.href='?opt_menu={$smarty.get.opt_menu}&opt_seccion={$smarty.get.opt_seccion}&opt_subseccion=delete&cod={$campos.id_item}'" title="Eliminar"  src="../../libs/imagenes/delete.gif">
                    </td>
                    <td>
                        {if ($campos.total_inventario > $campos.existencia_min) }
                            <img title="OK" src="../../libs/imagenes/ico_ok.gif">
                        {/if}

                        {if ($campos.total_inventario <= $campos.existencia_min) }
                            <img title="Existencia Bajo Minimos" src="../../libs/imagenes/ico_note_1.gif">
                        {/if}
                    </td>
                </tr>
