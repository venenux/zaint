...
//exit(0);
    ...
    for ($i = 0; $i < (int) $_POST["input_cantidad_items"]; $i++) {        
        $detalle_item_instruccion = "
            INSERT INTO cotizacion_presupuesto_detalle (
                `id_cotizacion_presupuesto_detalle`, `id_cotizacion`, `id_item`, `_item_descripcion` ,
                `_item_cantidad`, `_item_preciosiniva`, `_item_descuento`, `_item_montodescuento` ,
                `_item_piva`, `_item_totalsiniva`, `_item_totalconiva`, `usuario_creacion`, `fecha_creacion`, `_item_almacen`
            )
            VALUES (
                NULL , '" . $id_facturaTrans . "', '" . $_POST["_item_codigo"][$i] . "', '" . $_POST["_item_descripcion"][$i] . "','"
                . $_POST["_item_cantidad"][$i] . "', '" . $_POST["_item_preciosiniva"][$i] . "','" . $_POST["_item_descuento"][$i] . "', '" 
                . $_POST["_item_montodescuento"][$i] . "','" . $_POST["_item_piva"][$i] . "', '" . $_POST["_item_totalsiniva"][$i] . "', '" 
                . $_POST["_item_totalconiva"][$i] . "', '" . $login->getUsuario() . "', CURRENT_TIMESTAMP, '" . $_POST["_item_almacen"][$i] . "'
            );";
    ...
    if ($factura->errorTransaccion == 1) {
        Msg::setMessage("<span style=\"color:#62875f;\"><img src=\"../../libs/imagenes/ico_ok.gif\"> Presupuesto Generado Exitosamente con el <b>Nro. " . $formateo_nro_factura . "</b><br><img src=\"../../libs/imagenes/monto.png\"> <b>Monto Total: " . number_format($_POST["input_totalizar_total_general"], 2, ",", ".") . " </b><br><img src=\"../../libs/imagenes/cancelar.png\"> <b>Monto Cancelado: " . number_format($_POST["input_totalizar_monto_cancelar"], 2, ",", ".") . " </b><br><img src=\"../../libs/imagenes/monto.png\"><b>Monto Retencion: " . number_format($_POST["totalizar_total_retencion"], 2, ",", ".") . " </b><br><img src=\"../../libs/imagenes/ico_view.gif\"> <b><span style=\"color:red;\">Monto Pendiente: " . number_format($_POST["input_totalizar_saldo_pendiente"], 2, ",", ".") . " </span></b><br><img src=\"../../libs/imagenes/cambio.png\"> <b>Monto Cambio: " . number_format($_POST["input_totalizar_cambio"], 2, ",", ".") . " </b><br>Para imprimir ".$_POST["descripcion"]." <a href=\"#\" onclick=\"window.open(\'../../reportes/rpt_presupuesto_cotizacion.php?codigo=$formateo_nro_factura\');\">haga click aqui</a></span>");
    }
    if ($factura->errorTransaccion == 0) {
        Msg::setMessage("<span style=\"color:red;\">Error al tratar de crear el Presupuesto.</span>");
    }
