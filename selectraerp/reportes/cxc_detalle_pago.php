...
class PDF extends FPDF {
	...
	function Header() {
		...
		$this->Image('../libs/imagenes/asys.jpg', 6 ,10, 35, 15,'JPG', '');
		//$this->Cell(80,0, "LIBRO DE VENTASSSS".$mes." ".substr($fecha,0,4),0,0,'C');
		$this->Cell(115,0,'DETALLE PAGO',0,0,'R');
		$this->Ln(6);
		$this->Cell(92,0,'Cliente:',0,0,'C');
		$this->Ln(6);
		$this->Cell(101,0,'Fecha Pago:',0,0,'C');
		$this->Ln(6);
		$this->Cell(91,0,'Banco:',0,0,'C');
		$this->Cell(80,0,'Tipo:',0,0,'C');
		$this->Ln(6);
		$this->Cell(94,0,'Numero:',0,0,'C');
		$this->Cell(73,0,'Monto:',0,0,'C');
	...
	function ChapterBody() {
		...
		$this->SetWidths(array(30,30,30,30,30,30,30));
		...
		$this->Row(array('Nro.Factura','Monto','Monto Pago','Ret. I.S.L.R.','Imp. Munc.', 'TOTAL'),1);
...
#$fechaz=$fecha."-01";
#$array_factura =   $comunes->ObtenerFilasBySqlSelect("SELECT f.*, c.nombre, c.direccion, c.nit,c.cod_cliente, c.telefonos,c.direccion, fd.*, c.rif FROM factura f inner join clientes c on c.id_cliente = f.id_cliente inner join factura_detalle fd on fd.id_factura = f.id_factura   where month(f.fechaFactura) = month('".$fechaz."') and year(f.fechaFactura) = year('".$fechaz."')"); 
