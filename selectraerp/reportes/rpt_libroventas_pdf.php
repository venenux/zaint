...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetAligns(array("C", "C", "C", "C", "C", "C", "C", "C", "C", "C", "R", "R", "R", "R", "R", "R"));
        #$this->SetFillColor(232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232, 232);
        $this->Row(array('Nro.', 'Fecha', utf8_decode('Nombre/Razón Social del Cliente'), 'CI/RIF/PAS','Factura',
            /* 'Nro. Control',  'Fact. Anulada', */ utf8_decode('Nota Débito'), utf8_decode('Nota Crédito'), /* 'Tipo Trans.',
              'Total Ventas con impuesto', */'Total Ventas con impuesto','Ventas NG', 'Base Imponible', '% ' . $this->datosgenerales[0]["nombre_impuesto_principal"], $this->datosgenerales[0]["nombre_impuesto_principal"], $this->datosgenerales[0]["nombre_impuesto_principal"] . ' Retenido', utf8_decode('Total Operac.')), 1);
    }
    ...
    function ChapterBody() {
        ...
        while ($this->array_factura[$i]) {
           ...
           // $totalconiva =$this->array_factura[$i]["cod_estatus"] == 3 ? (-1) * $this->array_factura[$i]["TotalTotalFactura"] : $this->array_factura[$i]["TotalTotalFactura"];
            if ($iva_factura>0){
                $gb=0;
            }else{
               $gb=$total_factura; 
            ...
            $this->SetLineWidth(0.1);
            $this->SetWidths(array(10, 15, 50, 16, 18, 12, 15,18, 18, 18, 18, 18, 18, 18, 18, 18));
            $this->SetAligns(array("C", "C", "L", "L", "C", "C", "C", "C", "C", "C", "R", "R", "R", "R", "R", "R"));
            $this->Row(array(($i + 1),
                $fecha->format($format = "d-m-Y"),
                $this->array_factura[$i]["nombre"],
                $this->array_factura[$i]["rif"],
                //$this->array_factura[$i]["impresora_serial"],
                //$this->array_factura[$i]["nroz"],
        ...
        $width = 10;
        $this->SetFont('Arial', 'B', 8);
        $this->SetWidths(array(176,30,18,18,18, 18, 18, 18, 18, 18));
        $this->SetAligns(array("L",  "R", "R", "R","R", "R", "R", "R", "R", "R"));
        $this->Row(array("",  "",  "T O T A L E S" ,"", "",  "", "", $totalConIva,$totalVentasNoGravadas, $totalBaseImponible, "N/A", $totalIva, $totalIvaRet, $totalVentasConIva), 1);
    }
