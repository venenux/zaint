...
if($(this).val()!=""){
            ...
            $.ajax({
                    ...
                    success: function(data){
                     campos = eval(data);
                       $("input[name='totalizar_monto_iva2']").val(campos[0].total_iva);
                    }
            });
...
if(datoscliente[0].calc_reten_impuesto_islr==1){
    $("input[name='totalizar_base_retencion']").val(tmp_subtotal.toFixed(2));
...
$("#opt_cheque, #opt_tarjeta, #opt_deposito").val(0).trigger("change");
...
$("input[name='PFactura2']").live("click", function(){
    ...
    total_acancelar= parseFloat(sum0) + parseFloat(sum1) + parseFloat(sum2)+ parseFloat(sum3);
        
//|| parseFloat(total_acancelar) < parseFloat($("input[name='totalizar_monto_cancelar']").val())   


if(parseFloat(total_acancelar)!=0){
