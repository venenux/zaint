var eventos_form = {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(i=0;i<=this.vcampos.length;i++){
                    $("#items").append("<option value='"+this.vcampos[i].id_item+"'>"+/*this.vcampos[i].cod_item+"	"+*/this.vcampos[i].descripcion1+"</option>");
                }
            }
        });
    },
    ...
    GenerarCompraX: function()
    {
		...
		if(num_factura==""){
			Ext.Msg.alert("Alerta!","Debe ingresar el Nº de Factura");
			return false;
		}

		num_control_factura = $("#num_control_factura").val();
		if(num_factura==""){
			Ext.Msg.alert("Alerta!","Debe ingresar el Nº de control Factura");
			return false;
		}
		num_factura = $("#num_factura").val();
		if(num_factura==""){
		...
		*/
        ...
        if(femision==""){
            Ext.Msg.alert("Alerta!","Debe Cargar la Fecha de Emisión");
            return false;
        }
