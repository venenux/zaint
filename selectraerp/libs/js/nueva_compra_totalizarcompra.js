...
$("#totalizar_descripcion_base_retencion").change(function(){


if($(this).val()!=""){
            $.ajax({
                    type: 'GET',
                    data: 'opt=impuesto_islr&cod_impuesto_islr='+$(this).val()+"&monto_islr="+$("input[name='totalizar_base_retencion']").val(),
                    url:  '../../libs/php/ajax/ajax.php',
                    beforeSend: function(){
                    },
                    success: function(data){
                     campos = eval(data);
                       $("input[name='totalizar_monto_retencion']").val(campos[0].total_islr);
                       $("input[name='totalizar_pbase_retencion']").val(campos[0].porcentaje);
                    }
            });
...
//SI para el cliente aplica retencion Islr
if(datoscliente[0].calc_reten_impuesto_islr==1){
    $("input[name='totalizar_base_retencion']").val(calculo.toFixed(2));
    resultadoretencion = parseFloat(tmp_subtotal) * (parseFloat($("input[name='totalizar_pbase_retencion']").val())/100);
    totalretencion = parseFloat(tmp_subtotal) - parseFloat(resultadoretencion);
    $("input[name='totalizar_monto_retencion']").val(totalretencion.toFixed(2));
}

if(datoscliente[0].calc_reten_impuesto_1x1000==1){
calculo = (parseFloat(tmp_subtotal.toFixed(2)) -  parseFloat(tmp_descuento.toFixed(2))) / 1000;

$("input[name='totalizar_monto_1x1000']").val(calculo.toFixed(2));
    ...
    $("input[name='totalizar_monto_efectivo']").val($("input[name='totalizar_total_general']").val());
        ...
        $(".tab").live("mouseover", function(){
           ...
           tabclick = $(this).attr("id");
            ...
            if (centinela==false){centinela=true;
...
$("input[name='totalizar_monto_cancelar']").click(function(){
 ...
 datoscliente = eval($("input[name='DatosCliente']").val());
  ...
  if(datoscliente[0]['calc_reten_impuesto_islr']==1){
    totalizar_monto_retencion = parseFloat($("#totalizar_monto_retencion").val());
    retenciones += parseFloat(totalizar_monto_retencion);}

  if(datoscliente[0]['calc_reten_impuesto_iva']==1){
    totalizar_monto_iva2 =      parseFloat($("#totalizar_monto_iva2").val());
    retenciones +=  parseFloat(totalizar_monto_iva2) ;}

  if(datoscliente[0]['calc_reten_impuesto_1x1000']==1){
    totalizar_monto_1x1000 =    parseFloat($("#totalizar_monto_1x1000").val());
    retenciones +=parseFloat(totalizar_monto_1x1000);}
...
pendiente= parseFloat(resultado)-parseFloat(retenciones);

        if(parseFloat($(this).val())<parseFloat(pendiente)){
        restante = parseFloat(pendiente) - parseFloat($(this).val());
