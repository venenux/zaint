...
if(isset($_POST['buscar']) || $tipob!=NULL){
	...
	switch($tipob){
		...
		if ($rsac == '4') 
		{
			$var_sql="update requisiciones set situacion='Administracion' WHERE cod_requisicion = $id";
...
<?
	...
	titulo($modulo.": ".$cod_unidad.". ".$descripcion_u." - ".$cod_centro.": ".$descripcion_c,"requisiciones_add3.php?codigo=".$cod_unidad."&cod_centro=".$cod_centro,"centros_list2.php?codigo=".$cod_unidad,"12");
?>
...
<?
	...
	foreach($indices as $campo){
		...
		else if($nom_tabla=="concepto")
		{
			...
			$cod_proveedor=$fil["cod_proveedor"];
			if($cod_proveedor<>0){
	...
	switch($situacion)
	{
		...
		case "Registrada":
			iconoNuevopdf("requisicionespdf.php?id=".$id."&cod_centro=".$cod_centro, "Imprimir Requisicion","ico_print.gif");
			icono("requisiciones_edit3.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina, "Editar Requisicion", "ico_edit.gif");
			icono("javascript:confirmar('Desea Anular Esta Requisicion?','requisiciones3.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina."&rsac=6')", "Anular Requisicion", "ico_cancel.gif");
			...
			{
				icono("requisiciones3.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina."&rsac=4", "Enviar a Compras", "ico_ok.gif");
			}
		...
		case "Revisar":
			iconoNuevopdf("requisicionespdf.php?id=".$id."&cod_centro=".$cod_centro, "Imprimir Requisicion","ico_print.gif");
			icono("requisiciones_ver3.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina, "Ver Requisicion", "ico_view.gif");
			?>
			...
			if($fila2['valor']==0)
			{			
				icono("requisiciones3.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina."&rsac=5", "Enviada a Compras/Regresar", "ico_propiedades.gif");
			}else{
				?>
				<td><img width="16" height="16" align="left" border="0" title="En Proceso" src="../imagenes/ico_est4.gif"/></td>
				<?
		...
		case "Administracion":
			...
			<td><img width="16" height="16" align="left" border="0" title="En Proceso" src="../imagenes/ico_est4.gif"/></td>
			<?
			icono("requisiciones2.php?id=".$id."&codigo=".$cod_unidad."&cod_centro=".$cod_centro."&pagina=".$pagina."&rsac=5", "Enviada a Compras/Regresar", "ico_propiedades.gif");
