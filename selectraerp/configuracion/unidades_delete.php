...
if(isset($_POST['aceptar'])){
	$consul="select * from centros where cod_unidad=".$_POST["codigo"];
	$result=query($consul,$conexion);
	$num=mysql_num_rows($result);
	if($num==0)
	{
		$consulta="DELETE FROM ".$tabla." where cod_unidad=".$_POST["codigo"];
	...
	{
		echo "<SCRIPT language=\"JavaScript\" type=\"text/javascript\">
		alert(\"La unidad tiene centros adcritos, No se puede borrar la unidad\")
		location.href=\"".$url.".php?pagina=1\"
		</SCRIPT>";
	}
