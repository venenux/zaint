...
$url="ordenes_list_directas";
$modulo="Emision de Ordenes";
$tabla="ordenes";
$titulos=array("Codigo","Codigo Ref.","Fecha","Tipo","Proveedor","Estado","Monto Orden","Saldo");
$indices=array("0","18","1","8","13","16","4","17");
...
if(isset($_POST['buscar']) || $tipob!=NULL){
	...
	$consulta="select * from ".$tabla." where cod_requi=0 or cod_requi=null";
}
...
<TABLE width="100%">
	<TR class="row-br">
		<TD><?titulo("Consulta General de Ordenes","","../menu_int.php?cod=159","86");?></TD>
	</TR>
</TABLE>
...
<?
	...
	switch($tipo)
	{
		case 1:
			...
			iconoNuevopdf("ordenesprintdirectaspdf.php?id=$cod_ord&desTipo=$desTipo", "Imprimir Orden","ico_print.gif");
	...
	if($estado=="Revisar"||$estado=="REVISAR")
	{
		...
		icono("ordenes_list_directas.php?codigo=".$cod_ord."&pagina=".$pagina."&rsac=4", "Anular", "ico_cancel.gif");
	...
	if(($estado=="Emitida"||$estado=="EMITIDA")&&($cotizacion==0))
	{
		...
		icono("ordenes_list_directas.php?codigo=".$cod_ord."&pagina=".$pagina."&rsac=5", "Regresar a Compras", "ico_ok.gif");
	}else if (($estado=="Emitida"||$estado=="EMITIDA")&&($cotizacion<>0))
	{
		icono("javascript:confirmar('Desea Anular Esta Orden ?','ordenes_list_directas.php?codigo=".$cod_ord."&pagina=".$pagina."&rsac=4')", "Anular Orden", "ico_cancel.gif");
	}else
	{
		?><td><img src="../imagenes/ico_est6.gif"/></td><?
	}
