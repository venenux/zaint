...
$url="emision_orden_compras";
$modulo="Emisión de Ordenes de Compras";
$tabla="requisiciones";
...
if(isset($_POST['buscar']) || $tipob!=NULL){
	...
	$consulta=$consul."and tipo = 1 and situacion='Revisar'";
	//ECHO $consulta;
}else{
	if($accion=='Anular')
	{
	...
	{
		$var_sql="update compra set estado='Anulado', saldo=0 WHERE codigo=".$cod_ord;
		$rs = query($var_sql,$conexion);
	}
	if ($accion=='Emitir')
	{
		$rs = query("SELECT monto_orden,codigo FROM ordenes where cod_requi='$codigo' AND codigo = '$cod_ord' and estado = 'Revisar'",$conexion);
		while ($row_rs = fetch_array($rs)) 
		{
			$monto_orden_adj=$row_rs['monto_orden'];
			//$cod_ord=$row_rs["codigo"];
			
			$var_sql="update compra set estado = 'Emitida' where id_compra = ".$cod_ord." and cod_requi=".$cod_requisicion." and estado <> 'Anulado'";			
			$result = query($var_sql, $conexion);		
		}

		$var_sql="update compra set estado = 'Emitida' where id_compra = ".$cod_ord." and estado <> 'Anulado'";			
		$result = query($var_sql, $conexion); 
	
		$var_sql="update requisiciones set situacion = 'Adjudicada' where cod_requisicion=$cod_requisicion";
		$result = query($var_sql, $conexion); 	
	}
$consulta="select * from ".$tabla." as r INNER JOIN cotizaciones as c on r.cod_requisicion = c.cod_requisicion and c.estatus='Revisar' where tipo = 1 and situacion='Revisar'  ORDER BY r.cod_requisicion";
}
...
<?
	...
	foreach($indices as $campo){
		...
		{
			$consulp="select cod_compra,estado from compra where cod_requi=".$fila["cod_requisicion"]." and estado<>'Anulado' and cod_cotizacion=".$fila["codigo"];
			//echo $consulp."<br>";
			$resultp=query($consulp,$conexion);
			$fil=fetch_array($resultp);
			$cod_ord=$fil["id_compra"];
			$estado=$fil["estado"];
			//echo $estado;
	...
	$consul="select * from compra where cod_requi='".$codigo."' and estado='Revisar'";
	$resul=query($consul,$conexion);
	$numero_ordenes=num_rows($resul);
	
	$consulo="select * from compra where cod_requi='".$codigo."' and cod_cotizacion='".$fila['codigo']."' and estado='Revisar'";
	$resulo=query($consulo,$conexion);
	$fila2=fetch_array($resulo);
	$cod_ord=$fila2["id_compra"];
	if (mysql_num_rows($resulo))
	{
		switch($tipo){
		case 1:
			iconoNuevopdf("ordenesprintpdf3.php?id=$cod_ord&desTipo=$desTipo", "Imprimir Orden","ico_print.gif");
			icono("javascript:confirmar('Desea Anular Esta Orden ?','emision_orden_compras.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=AnularCompra')", "Anular Orden de Compras", "ico_cancel.gif");
			if($numero_cotizaciones==$numero_ordenes)
			{
				icono("emision_orden_compras.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=Emitir","Enviar Las Ordenes a Presupuesto","ico_back.gif");
			}
	...
	{	
		...
		{
			icono("emision_orden_compras.php?cod_requisicion=".$codigo."&cod_cotizacion=".$fila['codigo']."&accion=Anular","Anular Asignacion de Proveedor","ico_cancel.gif");
		}
		?>
		<td><img src="../imagenes/ico_est6.gif"/></td>
		<?
		//icono("ordenesprint3.php?codigo=$codigo", "Imprimir Orden","ico_print.gif");
		icono("adjudicar_orden_compras.php?cod_requisicion=".$codigo."&codigo=".$fila["codigo"], "Adjudicar Orden", "ico_ok.gif");	
	}
