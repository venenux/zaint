...
$url="emision_orden_trabajo";
$modulo="Emisión de Ordenes de Trabajo";
$tabla="requisiciones";
...
$consulta="select * from ".$tabla." as r INNER JOIN cotizaciones as c on r.cod_requisicion = c.cod_requisicion and c.estatus='Revisar' where tipo = 6 and situacion='Revisar'  ORDER BY r.cod_requisicion";
}
...
<?
	...
	if (mysql_num_rows($resulo))
	{
		...
		case 7:
			...
			icono("javascript:confirmar('Desea Anular Esta Orden ?','emision_orden_trabajo.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=AnularCompra')", "Anular Orden de Compras", "ico_cancel.gif");
			if($numero_cotizaciones==$numero_ordenes)
			{
				icono("emision_orden_trabajo.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=Emitir","Enviar Las Ordenes a Presupuesto","ico_back.gif");
			}
	...
	{	
		...
		{
			icono("emision_orden_trabajo.php?cod_requisicion=".$codigo."&cod_cotizacion=".$fila['codigo']."&accion=Anular","Anular Asignacion de Proveedor","ico_cancel.gif");
		}
		?>
		<td><img src="../imagenes/ico_est6.gif"/></td>
		<?
		//icono("ordenesprint3.php?codigo=$codigo", "Imprimir Orden","ico_print.gif");
		icono("adjudicar_orden_trabajo.php?cod_requisicion=".$codigo."&codigo=".$fila["codigo"], "Adjudicar Orden", "ico_ok.gif");	
	}
