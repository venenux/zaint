...
$url="emision_orden_contratos";
$modulo="Emisión de Ordenes de Contratos";
$tabla="requisiciones";
...
if(isset($_POST['buscar']) || $tipob!=NULL){
	...
	$consulta=$consul."and tipo = 7 and situacion='Revisar'";
	//ECHO $consulta;
}else{
	if($accion=='Anular')
	{
...
$consulta="select * from ".$tabla." as r INNER JOIN cotizaciones as c on r.cod_requisicion = c.cod_requisicion and c.estatus='Revisar' where tipo = 7 and situacion='Revisar'  ORDER BY r.cod_requisicion";
}
...
<?
	...
	$consulo="select * from ordenes where cod_requi='".$codigo."' and cod_cotizacion='".$fila['codigo']."' and estado='Revisar'";
	...
	if (mysql_num_rows($resulo))
	{
		...
		case 7:
			...
			icono("javascript:confirmar('Desea Anular Esta Orden ?','emision_orden_contratos.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=AnularCompra')", "Anular Orden de Compras", "ico_cancel.gif");
			if($numero_cotizaciones==$numero_ordenes)
			{
				icono("emision_orden_contratos.php?cod_requisicion=".$codigo."&cod_orden=".$cod_ord."&accion=Emitir","Enviar Las Ordenes a Presupuesto","ico_back.gif");
			}
	...
	{	
		...
		{
			icono("emision_orden_contratos.php?cod_requisicion=".$codigo."&cod_cotizacion=".$fila['codigo']."&accion=Anular","Anular Asignacion de Proveedor","ico_cancel.gif");
		}
		?>
		<td><img src="../imagenes/ico_est6.gif"/></td>
		<?
		//icono("ordenesprint3.php?codigo=$codigo", "Imprimir Orden","ico_print.gif");
		icono("adjudicar_orden_contratos.php?cod_requisicion=".$codigo."&codigo=".$fila["codigo"], "Adjudicar Orden", "ico_ok.gif");	
	}
