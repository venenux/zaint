...
if($_POST['bandera'] == 1)
{
	...
	alert(\"NO FUE POSIBLE REALIZAR LA OPERACION!! \")
...
function checkMyForm(valor,num_reg,var_por_iva,dispo) 
{
...
//********************************************************************
	...
	if (result5 < 0)
	{
		alert("NO TIENE SUFICIENTE DISPONIBILIDAD PRESUPUESTARIA, NO PODRÁ REALIZAR ESTA OPERACIÓN")
...
<table  width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  ORDER BY cod_requisicion_det ",$Conn);
   ...
   {	
	...
	$rslk = query("SELECT Dispo,RecNo FROM cwprepar WHERE Sector = '".$fetch33['sel_sector']."' and Programa = '".$fetch33['sel_programa']."' and Actividad = '".$fetch33['sel_actividad']."' and Codigo = '".$fetch333['codigo_cuenta']."'",$Conn);
   ...
   ?>
	...
	<input type="hidden" name="Programa" id="Programa" value="<? echo $fetchlk['sel_programa']; ?>">
	<input type="hidden" name="Actividad" id="Actividad" value="<? echo $fetchlk['sel_actividad']; ?>">
	...
	<input id="cod_orden" name="cod_orden" type="hidden" value="<? echo $var_cod_orden; ?>">
          ...
          <td><input  name="iva_sino_opt<?php echo $con2; ?>" type="checkbox" id="iva_sino_opt<?php echo $con2; ?>" value="checkbox" onchange="checkMyForm(<?php echo $con2;?>,<?php echo $con ?>,<?php echo $var_por_iva; ?>,<? echo $fetchlk['Dispo']; ?>);">
